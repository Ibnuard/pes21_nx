"""Exercise the Cup-only final-result score bridge with host-side stubs."""
...
class CupResultWiringTests(unittest.TestCase):
    def test_only_completed_cup_result_is_recorded(self):
...
static uint32_t phase, cup_active, scores_ready = 1u, calls;
...
"""
        source += function(hooks, "match_result_cup_finished") + "\n"
        source += function(hooks, "match_result_record_cup_score") + "\n"
        source += r"""
int main(void) {
