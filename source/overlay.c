...
static float measure_efootball_line(const char *text, int len, float gh,
                                    uint32_t weight) {
  return measure_efootball_line_mode(text, len, gh, weight, 0);
}

static int emit_efootball_line_mode(const char *text, int len,
                                    float x, float y, float gh,
                                    uint32_t weight, int full_resolution,
                                    GLfloat *verts) {
  if (weight >= EFOOTBALL_FONT_WEIGHTS)
    weight = EFOOTBALL_FONT_REGULAR;
...
static int emit_efootball_line(const char *text, int len, float x, float y,
  ...
  return emit_efootball_line_mode(text, len, x, y, gh, weight, 0, verts);
}
...
static int cup_fixture_is_bye(const CupTournament *cup, uint32_t round,
                              uint32_t index, const CupFixture *fixture) {
  if (!cup || !fixture) return 0;
  if (!round) {
    const uint32_t opening = cup->bracket_size / 2u;
    return index >= cup->team_count - opening;
  }
  return fixture->complete && fixture->home && !fixture->away;
}

static int emit_cup_slot_text(uint32_t team, uint32_t owner_hint,
                              int show_score, int has_score, uint32_t score,
  ...
  } else if (owner_hint <= COMPETITION_DRAFT_MAX_PLAYERS) {
    if (owner_hint)
      snprintf(label, sizeof(label), "P%u", owner_hint);
    else
      memcpy(label, "COM", 4);
  } else {
    memcpy(label, "TBD", 4);
  }
  const float glyph_h = roundf(fminf(height * 0.52f,
                                     (float)screen_height / 31.0f));
  const float text_x = x + 0.008f * (float)screen_width +
                       0.068f * (float)screen_height +
                       0.008f * (float)screen_width;
  const float text_y = y + (height - glyph_h) * 0.5f;
  const float plate_w = 0.038f * (float)screen_width;
  const float text_end = x + width -
      (show_score ? plate_w + 0.010f * (float)screen_width
                  : 0.010f * (float)screen_width);
  int count = emit_efootball_fit_line(
      label, (int)strlen(label), text_x, text_y,
      fmaxf(1.0f, text_end - text_x), glyph_h, glyph_h * 0.78f,
      EFOOTBALL_FONT_BOLD, verts);
  if (show_score) {
    char score_text[8];
    if (has_score && team)
    ...
    count += emit_efootball_line(
        ...
        x + width - 0.003f * (float)screen_width -
            (plate_w + score_w) * 0.5f,
        text_y, glyph_h, EFOOTBALL_FONT_BOLD, verts + count * 24);
  }
  return count;
}

static float cup_schedule_badge_x(float x, float width, int away) {
  const float inset = 0.010f * (float)screen_width;
  const float badge = 0.068f * (float)screen_height;
  return away ? x + width - inset - badge : x + inset;
}

static int emit_cup_schedule_match_text(uint32_t home, uint32_t away,
                                        uint32_t home_goals,
                                        uint32_t away_goals,
                                        int complete,
                                        float x, float y, float width,
                                        float height, GLfloat *verts) {
  char home_code[4], away_code[4], home_score[8], away_score[8];
  ...
  if (complete) {
    snprintf(home_score, sizeof(home_score), "%u", home_goals);
    snprintf(away_score, sizeof(away_score), "%u", away_goals);
  } else {
    memcpy(home_score, "-", 2);
    memcpy(away_score, "-", 2);
  }
  const float glyph_h = roundf(fminf(height * 0.43f,
                                     (float)screen_height / 31.0f));
  const float text_y = y + (height - glyph_h) * 0.5f;
  const float badge = 0.068f * (float)screen_height;
  const float name_gap = 0.007f * (float)screen_width;
  const float score_gap = 0.014f * (float)screen_width;
  const float home_x = cup_schedule_badge_x(x, width, 0) + badge + name_gap;
  const float away_right = cup_schedule_badge_x(x, width, 1) - name_gap;
  const float home_score_x = x + width * 0.395f;
  const float away_score_x = x + width * 0.605f;
  int count = emit_efootball_fit_line(
      home_code, 3, home_x, text_y,
      fmaxf(1.0f, home_score_x - score_gap - home_x),
      glyph_h, glyph_h * 0.85f, EFOOTBALL_FONT_BOLD, verts);
  count += emit_efootball_centered_fit_line(
      home_score, (int)strlen(home_score), home_score_x, text_y,
      width * 0.08f, glyph_h, glyph_h * 0.80f,
      EFOOTBALL_FONT_BOLD, verts + count * 24);
  count += emit_efootball_centered_fit_line(
      "VS", 2, x + width * 0.5f, text_y,
      width * 0.11f, glyph_h * 0.90f, glyph_h * 0.75f,
      EFOOTBALL_FONT_BOLD, verts + count * 24);
  count += emit_efootball_centered_fit_line(
      away_score, (int)strlen(away_score), away_score_x, text_y,
      width * 0.08f, glyph_h, glyph_h * 0.80f,
      EFOOTBALL_FONT_BOLD, verts + count * 24);
  count += emit_efootball_right_fit_line(
      away_code, 3, away_right, text_y,
      fmaxf(1.0f, away_right - away_score_x - score_gap),
      glyph_h, glyph_h * 0.85f, EFOOTBALL_FONT_BOLD,
      verts + count * 24);
  return count;
}
...
/* FL26 emblems remain beside the user's runtime, never inside the public NRO.
 * Upload each selected Cup at most once so menu redraws never hit SD or PNG
 * decoding again (including when an optional emblem is missing). */
static void prepare_cup_logo_asset(int active) {
  if (!active) return;
  const uint32_t index = competition_frontend_cup_catalog_index();
  const char *name = competition_frontend_cup_logo_file();
  if (index >= FL26_CUP_CATALOG_COUNT || !name ||
      gl.cup_logo_attempted[index])
    return;
  gl.cup_logo_attempted[index] = 1;
  ...
  free(bytes);
  if (!gl.cup_logo_uploaded[index])
    debugPrintf("[overlay] invalid Cup logo: %s\n", path);
}

static GLuint current_cup_logo_texture(void) {
  const uint32_t index = competition_frontend_cup_catalog_index();
  return index < FL26_CUP_CATALOG_COUNT && gl.cup_logo_uploaded[index]
...
typedef struct {
  int backdrop, shell, header, left, right, left_bar, right_bar;
  int shade[6], ornament[4];
  int rows[5], schedule[4], parent[2], connector_first, connector_count;
  int owner_badge[5], owner_badge_count;
  int owner_text_first, owner_text_count;
  int scorer_portrait_quad[4];
  uint32_t scorer_portrait_id[4], scorer_count;
  int action[4], action_frame[4], action_gloss[4], trophy;
  ...
  float shoulder_centers[2], shoulder_y, shoulder_size;
  float schedule_top;
  uint32_t shoulder_count;
  uint32_t row_count, schedule_count, action_count, helper_count_keys;
  int editing, complete;
  RoundedRectStyle shell_style, header_style, panel_style, bar_style;
  RoundedRectStyle right_panel_style, right_bar_style, champion_style;
  RoundedRectStyle row_style, schedule_style, parent_style, action_style;
  RoundedRectStyle knockout_row_style;
  RoundedRectStyle frame_style, owner_style;
} LeagueHubRender;

static void league_emit_text(const char *line, float x, float y, float max_w,
                             float glyph_h, uint32_t weight, int alignment,
                             GLfloat *verts, int *quads) {
...
static void league_emit_left(const char *line, float x, float y, float max_w,
                             float glyph_h, GLfloat *verts, int *quads) {
  league_emit_text(line, x, y, max_w, glyph_h, EFOOTBALL_FONT_BOLD, 0,
                   verts, quads);
}

static void league_emit_center(const char *line, float x, float y, float max_w,
                               float glyph_h, GLfloat *verts, int *quads) {
  league_emit_text(line, x, y, max_w, glyph_h, EFOOTBALL_FONT_BOLD, 1,
                   verts, quads);
}

static void league_emit_stencil_center(const char *line, float x, float y,
                                       float max_w, float glyph_h,
                                       GLfloat *verts, int *quads) {
  league_emit_text(line, x, y, max_w, glyph_h, EFOOTBALL_FONT_STENCIL, 1,
                   verts, quads);
}

static void league_emit_right(const char *line, float x, float y, float max_w,
                              float glyph_h, GLfloat *verts, int *quads) {
  league_emit_text(line, x, y, max_w, glyph_h, EFOOTBALL_FONT_BOLD, 2,
                   verts, quads);
}

static void league_hub_emit(LeagueHubRender *ui, GLfloat *verts, int *quads) {
  ...
  const float shell_x = 0.055f * sw, shell_y = 0.070f * sh;
  const float shell_w = 0.890f * sw, shell_h = 0.690f * sh;
  const float left_x = shell_x + 0.019f * sw;
  const float left_w = 0.500f * sw;
  const float right_x = left_x + left_w + 0.018f * sw;
  const float right_w = shell_x + shell_w - 0.019f * sw - right_x;
  const float body_y = shell_y + 0.132f * sh;
  const float body_h = 0.540f * sh;
  const float bar_y = body_y + 0.010f * sh, bar_h = 0.054f * sh;
  const float row_x = left_x + 0.012f * sw;
  const float row_w = left_w - 0.024f * sw;
  const float row_h = 0.069f * sh;
  const float sched_x = right_x + 0.012f * sw;
  const float sched_w = right_w - 0.024f * sw;
  const float sched_h = 0.092f * sh;
  const float glyph_h = fmaxf(20.0f, sh / 35.0f);
  const float badge_h = 0.059f * sh;
  const uint32_t table_page = competition_frontend_league_table_page();
  const uint32_t bracket_round = competition_frontend_league_bracket_round();
  const uint32_t bracket_page = competition_frontend_league_bracket_page();
  uint32_t row_team[5] = {0};
  ...
  ui->backdrop = *quads;
  *quads += emit_image_rect(0, 0, sw, sh, verts + *quads * 24);
  ui->shell_style = (RoundedRectStyle){shell_w, shell_h, 0.023f * sh};
  ui->header_style = (RoundedRectStyle){shell_w, 0.105f * sh, 0.023f * sh};
  ui->panel_style = (RoundedRectStyle){left_w, body_h, 0.015f * sh};
  ui->right_panel_style = (RoundedRectStyle){right_w, body_h, 0.015f * sh};
  ui->bar_style = (RoundedRectStyle){left_w - 0.016f * sw, bar_h, 0.010f * sh};
  ui->right_bar_style = (RoundedRectStyle){right_w - 0.016f * sw,
      bar_h, 0.010f * sh};
  ui->champion_style = (RoundedRectStyle){row_w - 0.192f * sw,
      0.240f * sh, 0.014f * sh};
  ui->row_style = (RoundedRectStyle){row_w, row_h, 0.008f * sh};
  ui->owner_style = (RoundedRectStyle){0.048f * sw,
      0.041f * sh, 0.0205f * sh};
  ui->knockout_row_style = (RoundedRectStyle){0.226f * sw,
      row_h, 0.008f * sh};
  ui->schedule_style = (RoundedRectStyle){sched_w, sched_h, 0.008f * sh};
  ui->parent_style = (RoundedRectStyle){0.177f * sw, row_h, 0.008f * sh};
  ui->action_style = (RoundedRectStyle){0.162f * sw, 0.075f * sh,
                                        0.013f * sh};
  ui->frame_style = (RoundedRectStyle){0.167f * sw, 0.081f * sh,
                                       0.015f * sh};
  ui->shell = *quads;
  *quads += emit_round_rect_quad(shell_x, shell_y, shell_w, shell_h,
                                  verts + *quads * 24);
  ui->header = *quads;
  *quads += emit_round_rect_quad(shell_x, shell_y, shell_w, 0.105f * sh,
                                  verts + *quads * 24);
  ui->left = *quads;
  *quads += emit_round_rect_quad(left_x, body_y, left_w, body_h,
                                  verts + *quads * 24);
  ui->right = *quads;
  *quads += emit_round_rect_quad(right_x, body_y, right_w, body_h,
                                  verts + *quads * 24);
  ui->left_bar = *quads;
  *quads += emit_round_rect_quad(left_x + 0.008f * sw, bar_y,
      left_w - 0.016f * sw, bar_h, verts + *quads * 24);
  ui->right_bar = *quads;
  *quads += emit_round_rect_quad(right_x + 0.008f * sw, bar_y,
      right_w - 0.016f * sw, bar_h, verts + *quads * 24);
  for (uint32_t band = 0; band < 3u; band++) {
    const float shade_y = body_y + (0.065f + 0.075f * band) * sh;
    ui->shade[band * 2u] = *quads;
    *quads += emit_rect(left_x + 0.002f * sw, shade_y,
        left_w - 0.004f * sw, 0.075f * sh, verts + *quads * 24);
    ui->shade[band * 2u + 1u] = *quads;
    *quads += emit_rect(right_x + 0.002f * sw, shade_y,
        right_w - 0.004f * sw, 0.075f * sh, verts + *quads * 24);
  }
  const float ornament_w = 0.039f * sw;
  const float ornament_h = 0.040f * sh;
  const float ornament_y = bar_y + 0.006f * sh;
  const float ornament_panel_x[2] = {left_x + 0.008f * sw,
                                      right_x + 0.008f * sw};
  const float ornament_panel_w[2] = {left_w - 0.016f * sw,
                                      right_w - 0.016f * sw};
  for (uint32_t panel = 0; panel < 2u; panel++) {
    const float inset = (panel ? 0.006f : 0.055f) * sw;
    const uint32_t index = panel * 2u;
    ui->ornament[index] = *quads;
    *quads += emit_image_rect_uv(ornament_panel_x[panel] + inset,
        ornament_y, ornament_w, ornament_h, 0.0f, 0.0f, 1.0f, 1.0f,
        verts + *quads * 24);
    ui->ornament[index + 1u] = *quads;
    *quads += emit_image_rect_uv(ornament_panel_x[panel] +
        ornament_panel_w[panel] - ornament_w - inset,
        ornament_y, ornament_w, ornament_h, 1.0f, 0.0f, 0.0f, 1.0f,
        verts + *quads * 24);
  }

  if (table_phase) {
    uint8_t ranked[LEAGUE_MAX_TEAMS] = {0};
    if (valid) league_tournament_ranked_slots(league, ranked);
    const uint32_t total = draft ? draft->team_count : 0u;
    for (uint32_t i = 0; i < 5u && table_page * 5u + i < total; i++) {
      const uint32_t slot = valid && !ui->editing
          ? ranked[table_page * 5u + i] : table_page * 5u + i;
      row_slot[i] = slot;
      row_team[i] = draft->teams[slot];
      ui->rows[i] = *quads;
      *quads += emit_round_rect_quad(row_x,
          body_y + (0.130f + 0.077f * i) * sh,
          row_w, row_h, verts + *quads * 24);
      ui->row_count++;
    }
    for (uint32_t i = 0; i < ui->row_count; i++) {
      if (!draft->owners[row_slot[i]]) continue;
      ui->owner_badge[i] = *quads;
      *quads += emit_round_rect_quad(row_x + row_w - 0.208f * sw,
          body_y + (0.130f + 0.077f * i) * sh +
              (row_h - ui->owner_style.height) / 2,
          ui->owner_style.width, ui->owner_style.height,
          verts + *quads * 24);
    ...
    if (valid && league->active_matchday < league->matchday_count) {
      ...
      for (uint32_t i = 0; i < 4u && schedule_page * 4u + i < count; i++) {
        const LeagueFixture *fixture = league_tournament_matchday_fixture(
            league, day, schedule_page * 4u + i);
        if (!fixture) continue;
        sched_home[i] = fixture->home;
        sched_away[i] = fixture->away;
        sched_hg[i] = fixture->home_goals;
        sched_ag[i] = fixture->away_goals;
        sched_done[i] = fixture->complete;
        ui->schedule_count++;
      }
    }
  } else if (!champion_page) {
    const CupTournament *cup = &league->knockout;
    const uint32_t first = bracket_page * 2u;
    const uint32_t count = cup_tournament_fixture_count(cup, bracket_round);
    const float child_x = row_x, child_w = 0.226f * sw;
    const float parent_x = row_x + row_w - ui->parent_style.width;
    for (uint32_t i = 0; i < 2u && first + i < count; i++) {
      const CupFixture *fixture = cup_tournament_fixture(cup,
          bracket_round, first + i);
      if (!fixture) continue;
      row_team[i * 2u] = fixture->home;
      row_team[i * 2u + 1u] = fixture->away;
      for (uint32_t side = 0; side < 2u; side++) {
        const uint32_t index = i * 2u + side;
        ui->rows[index] = *quads;
        *quads += emit_round_rect_quad(child_x,
            body_y + (0.165f + i * 0.190f + side * 0.083f) * sh,
            child_w, row_h, verts + *quads * 24);
        ui->row_count++;
      }
      ui->parent[i] = *quads;
      *quads += emit_round_rect_quad(parent_x,
          body_y + (0.207f + i * 0.190f) * sh,
          ui->parent_style.width, row_h, verts + *quads * 24);
      parent_team[i] = fixture->winner;
      sched_home[i] = fixture->home;
      sched_away[i] = fixture->away;
      sched_hg[i] = fixture->home_goals;
      sched_ag[i] = fixture->away_goals;
    ...
    ui->connector_first = *quads;
    for (uint32_t i = 0; i < 2u && ui->parent[i]; i++) {
      const float mid_x = child_x + child_w + 0.017f * sw;
      const float child_mid1 = body_y + (0.165f + i * 0.190f) * sh + row_h / 2;
      const float child_mid2 = child_mid1 + 0.083f * sh;
      const float parent_mid = (child_mid1 + child_mid2) / 2;
      const float thick = fmaxf(1.8f, sh / 390.0f);
      *quads += emit_rect(child_x + child_w, child_mid1,
          mid_x - child_x - child_w, thick, verts + *quads * 24);
      *quads += emit_rect(child_x + child_w, child_mid2,
          mid_x - child_x - child_w, thick, verts + *quads * 24);
      *quads += emit_rect(mid_x, child_mid1, thick,
          child_mid2 - child_mid1, verts + *quads * 24);
      *quads += emit_rect(mid_x, parent_mid,
          parent_x - mid_x, thick, verts + *quads * 24);
      ui->connector_count += 4;
    }
  } else {
    const uint32_t champion = league->knockout.champion;
    row_team[0] = champion;
    ui->rows[0] = *quads;
    *quads += emit_round_rect_quad(row_x + 0.096f * sw,
        body_y + 0.172f * sh, row_w - 0.192f * sw,
        0.240f * sh, verts + *quads * 24);
    ui->row_count = 1u;
    ui->trophy = *quads;
    *quads += emit_image_rect(row_x + row_w - 0.084f * sw,
        body_y + 0.183f * sh, 0.080f * sw, 0.210f * sh,
        verts + *quads * 24);
    const CupTournament *cup = &league->knockout;
    for (uint32_t i = 0; i < 4u && i < cup->history_count; i++) {
      const uint32_t history = cup->history_count - 1u - i;
      const CupFixture *fixture = cup_tournament_fixture(cup,
          cup->history_round[history], cup->history_index[history]);
      if (!fixture) continue;
      sched_home[i] = fixture->home;
      sched_away[i] = fixture->away;
      sched_hg[i] = fixture->home_goals;
  ...
  if (scorer_panel && valid) {
    ui->scorer_count = league_tournament_top_scorers(league, scorer_slots);
    ui->schedule_count = ui->scorer_count;
  }

  {
    ...
    for (uint32_t i = 0; i < ui->schedule_count; i++) {
      ui->schedule[i] = *quads;
      *quads += emit_round_rect_quad(sched_x, top + i * step,
          sched_w, sched_h, verts + *quads * 24);
    }
  }
  ui->action_count = ui->complete ? 1u : 4u;
  const float action_w = ui->action_style.width;
  const float action_gap = 0.011f * sw;
  const float action_x = (sw - ui->action_count * action_w -
      (ui->action_count - 1u) * action_gap) * 0.5f;
  const float action_y = 0.802f * sh;
  for (uint32_t i = 0; i < ui->action_count; i++) {
    const float x = action_x + i * (action_w + action_gap);
    ui->action_frame[i] = *quads;
    *quads += emit_round_rect_quad(x - 0.0025f * sw,
        action_y - 0.003f * sh, ui->frame_style.width,
        ui->frame_style.height, verts + *quads * 24);
    ui->action[i] = *quads;
    *quads += emit_round_rect_quad(x, action_y, action_w,
        ui->action_style.height, verts + *quads * 24);
    ui->action_gloss[i] = *quads;
    *quads += emit_rect(x + 0.006f * sw, action_y + 0.007f * sh,
        action_w - 0.012f * sw, 0.017f * sh, verts + *quads * 24);
  }

  ui->badge_first = *quads;
  if (table_phase) {
    for (uint32_t i = 0; i < ui->row_count; i++) if (row_team[i]) {
      *quads += emit_badge(exhibition_team_catalog_badge(row_team[i]),
          row_x + 0.042f * sw,
          body_y + (0.130f + i * 0.077f) * sh + (row_h - badge_h) / 2,
          badge_h, badge_h, verts + *quads * 24);
      ui->badge_count++;
    }
  } else if (!champion_page) {
    for (uint32_t i = 0; i < ui->row_count; i++) if (row_team[i]) {
      *quads += emit_badge(exhibition_team_catalog_badge(row_team[i]),
          row_x + 0.006f * sw,
          body_y + (0.165f + (i / 2u) * 0.190f +
              (i & 1u) * 0.083f) * sh + (row_h - badge_h) / 2,
          badge_h, badge_h, verts + *quads * 24);
      ui->badge_count++;
    }
    for (uint32_t i = 0; i < 2u; i++) if (parent_team[i]) {
      *quads += emit_badge(exhibition_team_catalog_badge(parent_team[i]),
          row_x + row_w - ui->parent_style.width + 0.006f * sw,
          body_y + (0.207f + i * 0.190f) * sh + (row_h - badge_h) / 2,
          badge_h, badge_h, verts + *quads * 24);
      ui->badge_count++;
    }
  } else if (row_team[0]) {
    const float big = 0.135f * sh;
    *quads += emit_badge(exhibition_team_catalog_badge(row_team[0]),
        row_x + (row_w - big) / 2 - 0.001f * sw,
        body_y + 0.193f * sh, big, big, verts + *quads * 24);
    ui->badge_count++;
  }
  if (scorer_panel && valid) {
    for (uint32_t i = 0; i < ui->scorer_count; i++) {
      const LeagueScorer *scorer = &league->scorers[scorer_slots[i]];
      const float y = body_y + (ui->schedule_top + 0.103f * i) * sh +
          (sched_h - badge_h) / 2;
      *quads += emit_badge(exhibition_team_catalog_badge(scorer->team),
          sched_x + sched_w - 0.088f * sw, y, badge_h, badge_h,
          verts + *quads * 24);
      ui->badge_count++;
      ui->scorer_portrait_id[i] = scorer->portrait_id;
    }
  } else {
    for (uint32_t i = 0; i < ui->schedule_count; i++) {
      const float y = body_y + (0.119f + 0.103f * i) * sh +
          (sched_h - badge_h) / 2;
      if (sched_home[i]) {
        *quads += emit_badge(exhibition_team_catalog_badge(sched_home[i]),
            sched_x + 0.010f * sw, y, badge_h, badge_h,
            verts + *quads * 24);
        ui->badge_count++;
      }
      if (sched_away[i]) {
        *quads += emit_badge(exhibition_team_catalog_badge(sched_away[i]),
            sched_x + sched_w - 0.010f * sw - badge_h, y,
            badge_h, badge_h, verts + *quads * 24);
        ui->badge_count++;
      }
    }
  ...
  if (scorer_panel && valid) {
    for (uint32_t i = 0; i < ui->scorer_count; i++) {
      const float y = body_y + (ui->schedule_top + 0.103f * i) * sh +
          (sched_h - badge_h) / 2;
      ui->scorer_portrait_quad[i] = *quads;
      *quads += emit_image_rect(sched_x + 0.010f * sw, y,
          badge_h, badge_h, verts + *quads * 24);
    }
  }
  ...
  ui->bar_first = *quads;
  const char *left_title = table_phase ? "LEAGUE TABLE" :
      champion_page ? "CHAMPION" : "KNOCKOUT BRACKET";
  league_emit_stencil_center(left_title, left_x + left_w / 2,
      bar_y + 0.012f * sh, left_w * 0.70f, sh / 34,
      verts, quads);
  if (!ui->editing && !ui->complete) {
    ui->shoulder_count = 2u;
    ui->shoulder_centers[0] = left_x + 0.025f * sw;
    ui->shoulder_centers[1] = left_x + left_w - 0.025f * sw;
    ui->shoulder_y = bar_y + bar_h * 0.5f;
    ui->shoulder_size = 0.032f * sh;
  }
  league_emit_stencil_center(scorer_panel ? "TOP SCORER" : "MATCH SCHEDULE",
      right_x + right_w / 2, bar_y + 0.012f * sh,
      right_w * 0.83f, sh / 34, verts, quads);
  ui->bar_count = *quads - ui->bar_first;

  ui->body_first = *quads;
  if (table_phase) {
    const uint32_t total = draft ? draft->team_count : 0u;
    snprintf(caption, sizeof(caption), "STANDINGS  %u / %u",
        total ? table_page + 1u : 1u, total ? (total + 4u) / 5u : 1u);
    league_emit_left(caption, row_x, body_y + 0.080f * sh,
        0.240f * sw, sh / 41, verts, quads);
    const float stat_centers[6] = {
        row_x + row_w - 0.1375f * sw,
        row_x + row_w - 0.1133f * sw,
        row_x + row_w - 0.0891f * sw,
        row_x + row_w - 0.0648f * sw,
        row_x + row_w - 0.0375f * sw,
        row_x + row_w - 0.0110f * sw};
    const float stat_widths[6] = {
        0.027f * sw, 0.027f * sw, 0.027f * sw,
        0.027f * sw, 0.045f * sw, 0.040f * sw};
    static const char *const stat_headers[6] = {
        "P", "W", "D", "L", "GD", "PTS"};
    for (uint32_t stat = 0; stat < 6u; stat++)
      league_emit_center(stat_headers[stat], stat_centers[stat],
          body_y + 0.080f * sh, stat_widths[stat], sh / 43,
          verts, quads);
    for (uint32_t i = 0; i < ui->row_count; i++) {
      const uint32_t slot = row_slot[i];
      const float y = body_y + (0.130f + i * 0.077f) * sh +
          (row_h - glyph_h) / 2;
      snprintf(caption, sizeof(caption), "%u", table_page * 5u + i + 1u);
      league_emit_center(caption, row_x + 0.021f * sw, y,
          0.034f * sw, glyph_h, verts, quads);
      if (row_team[i]) {
        const char *name = exhibition_team_catalog_name(row_team[i]);
        league_emit_left(name, row_x + 0.042f * sw + badge_h +
            0.009f * sw, y,
            draft->owners[slot] ? 0.178f * sw : 0.235f * sw,
            glyph_h, verts, quads);
      } else {
        snprintf(caption, sizeof(caption), "%s",
            draft && draft->owners[slot] ? "SELECT TEAM" : "COM TEAM");
        league_emit_left(caption, row_x + 0.105f * sw, y,
            0.145f * sw, glyph_h, verts, quads);
      }
      if (valid) {
        const LeagueStanding *st = &league->standings[slot];
        const int32_t values[6] = {st->played, st->wins, st->draws,
            st->losses, (int32_t)st->goals_for - st->goals_against,
            st->points};
        for (uint32_t stat = 0; stat < 6u; stat++) {
          if (stat == 4u)
            snprintf(caption, sizeof(caption), "%+d", values[stat]);
          else
            snprintf(caption, sizeof(caption), "%d", values[stat]);
          league_emit_center(caption, stat_centers[stat], y,
              stat_widths[stat], glyph_h, verts, quads);
        }
      }
    }
  } else if (!champion_page) {
    const CupTournament *cup = &league->knockout;
    snprintf(caption, sizeof(caption), "ROUND %u / %u    PAGE %u",
        bracket_round + 1u, cup->round_count, bracket_page + 1u);
    league_emit_left(caption, row_x, body_y + 0.081f * sh,
        row_w, sh / 39, verts, quads);
    for (uint32_t i = 0; i < ui->row_count; i++) {
      char code[4];
      const float y = body_y + (0.165f + (i / 2u) * 0.190f +
          (i & 1u) * 0.083f) * sh + (row_h - glyph_h) / 2;
      if (row_team[i]) cup_team_code(row_team[i], code);
      else memcpy(code, "TBD", 4);
      league_emit_left(code, row_x + 0.006f * sw + badge_h +
          0.011f * sw, y, 0.075f * sw, glyph_h, verts, quads);
      const CupFixture *fixture = cup_tournament_fixture(cup,
          bracket_round, bracket_page * 2u + i / 2u);
      snprintf(caption, sizeof(caption), "%s",
          fixture && fixture->complete
              ? (i & 1u) ? "0" : "0" : "-");
      if (fixture && fixture->complete)
        snprintf(caption, sizeof(caption), "%u",
            (i & 1u) ? fixture->away_goals : fixture->home_goals);
      league_emit_right(caption, row_x + 0.216f * sw, y,
          0.035f * sw, glyph_h, verts, quads);
    }
    for (uint32_t i = 0; i < 2u && ui->parent[i]; i++) {
      char code[4];
      if (parent_team[i]) cup_team_code(parent_team[i], code);
      else memcpy(code, "TBD", 4);
      league_emit_center(code,
          row_x + row_w - ui->parent_style.width / 2,
          body_y + (0.207f + i * 0.190f) * sh +
              (row_h - glyph_h) / 2,
          ui->parent_style.width * 0.79f, glyph_h, verts, quads);
    }
  } else {
    league_emit_center(row_team[0]
            ? exhibition_team_catalog_name(row_team[0]) : "TBD",
        row_x + row_w / 2, body_y + 0.342f * sh,
        row_w * 0.48f, sh / 32, verts, quads);
    league_emit_center("CHAMPION", row_x + row_w / 2,
        body_y + 0.391f * sh, row_w * 0.45f, sh / 37,
        verts, quads);
  }
  if (scorer_panel) {
    if (!ui->scorer_count) {
      league_emit_center(valid && league->first_match_started
              ? "NO GOALS RECORDED YET" : "NO MATCHES PLAYED YET",
          right_x + right_w / 2, body_y + 0.240f * sh,
          right_w * 0.88f, sh / 34, verts, quads);
    } else {
      for (uint32_t i = 0; i < ui->scorer_count; i++) {
        const LeagueScorer *scorer = &league->scorers[scorer_slots[i]];
        const float y = body_y + (ui->schedule_top + 0.103f * i) * sh +
            (sched_h - glyph_h) / 2;
        snprintf(caption, sizeof(caption), "%u  %s", i + 1u,
                 scorer->name);
        league_emit_left(caption, sched_x + 0.015f * sw + badge_h,
            y, sched_w - badge_h - 0.084f * sw, glyph_h,
            verts, quads);
        snprintf(caption, sizeof(caption), "%u", scorer->goals);
        league_emit_right(caption, sched_x + sched_w - 0.015f * sw,
            y, 0.043f * sw, glyph_h, verts, quads);
      }
    }
  } else {
    ...
    for (uint32_t i = 0; i < ui->schedule_count; i++) {
      char home[4] = "TBD", away[4] = "TBD";
      if (sched_home[i]) cup_team_code(sched_home[i], home);
      if (sched_away[i]) cup_team_code(sched_away[i], away);
      const float y = body_y + (0.119f + 0.103f * i) * sh +
          (sched_h - glyph_h) / 2;
      league_emit_left(home, sched_x + 0.011f * sw + badge_h +
          0.006f * sw, y, 0.060f * sw, glyph_h,
          verts, quads);
      league_emit_right(away, sched_x + sched_w - badge_h -
          0.018f * sw, y, 0.060f * sw, glyph_h,
          verts, quads);
      if (sched_done[i])
        snprintf(caption, sizeof(caption), "%u  -  %u",
  ...
  ui->owner_text_first = *quads;
  if (table_phase && draft) {
    for (uint32_t i = 0; i < ui->row_count; i++) {
      const uint8_t owner = draft->owners[row_slot[i]];
      if (!owner) continue;
      snprintf(caption, sizeof(caption), "P%u", owner);
      league_emit_center(caption,
          row_x + row_w - 0.208f * sw + ui->owner_style.width / 2,
          body_y + (0.130f + 0.077f * i) * sh +
              (row_h - sh / 46.0f) / 2,
          ui->owner_style.width * 0.90f, sh / 46.0f,
          verts, quads);
    }
  }
  ...
  for (uint32_t i = 0; i < ui->action_count; i++) {
    ...
    league_emit_center(label, action_x + i * (action_w + action_gap) +
        action_w / 2, action_y + 0.023f * sh,
        action_w * 0.90f, sh / 39, verts, quads);
    ui->action_text_count[i] = *quads - ui->action_text_first[i];
  }
  ui->helper_first = *quads;
  const char *keys[4] = {"A", "B", ui->editing ? "X" : "Y", "Y"};
  const char *view_label = scorer_panel ? "MATCH SCHEDULE" : "TOP SCORER";
  const char *labels[4] = {ui->complete ? "CONFIRM" :
      ui->editing ? "SELECT TEAM" : "SELECT",
      ui->editing ? "BACK" : "TOP TO MENU",
      ui->editing ? "RANDOM TEAM" : view_label,
      view_label};
  ui->helper_count_keys = ui->complete ? 1u : ui->editing ? 4u : 3u;
  ui->helper_y = 0.945f * sh;
  ui->helper_size = 0.034f * sh;
  const float helper_text_h = roundf(sh / 43.0f);
  float helper_x = shell_x + 0.012f * sw;
  for (uint32_t i = 0; i < ui->helper_count_keys; i++) {
    ui->helper_keys[i] = keys[i];
    ui->helper_centers[i] = helper_x + ui->helper_size * 0.5f;
    const float label_x = helper_x + ui->helper_size + 0.008f * sw;
    league_emit_left(labels[i], label_x,
...
static void league_hub_draw(const LeagueHubRender *ui, uint32_t focus) {
  ...
  glBindTexture(GL_TEXTURE_2D, gl.league_hub_stadium_tex);
  glDrawArrays(GL_TRIANGLES, ui->backdrop * 6, 6);
  glBindTexture(GL_TEXTURE_2D, gl.tex);
  glUniform1f(gl.loc_image, 0.0f);
  glUniform1f(gl.loc_solid, 1.0f);
  use_rounded_rect(&ui->shell_style);
  glUniform4f(gl.loc_color, 0.84f, 0.91f, 0.98f, 0.97f);
  glDrawArrays(GL_TRIANGLES, ui->shell * 6, 6);
  use_rounded_rect(&ui->header_style);
  glUniform4f(gl.loc_color, 0.015f, 0.075f, 0.24f, 0.99f);
  glDrawArrays(GL_TRIANGLES, ui->header * 6, 6);
  use_rounded_rect(&ui->panel_style);
  glUniform4f(gl.loc_color, 0.94f, 0.97f, 1.0f, 0.98f);
  glDrawArrays(GL_TRIANGLES, ui->left * 6, 6);
  use_rounded_rect(&ui->right_panel_style);
  glDrawArrays(GL_TRIANGLES, ui->right * 6, 6);
  use_rounded_rect(NULL);
  for (uint32_t band = 0; band < 3u; band++) {
    glUniform4f(gl.loc_color, 0.52f, 0.70f, 0.96f,
                0.12f - 0.025f * band);
    glDrawArrays(GL_TRIANGLES, ui->shade[band * 2u] * 6, 12);
  }
  use_rounded_rect(&ui->bar_style);
  glUniform4f(gl.loc_color, 0.035f, 0.16f, 0.39f, 0.99f);
  glDrawArrays(GL_TRIANGLES, ui->left_bar * 6, 6);
  use_rounded_rect(&ui->right_bar_style);
  glDrawArrays(GL_TRIANGLES, ui->right_bar * 6, 6);
  use_rounded_rect(NULL);
  if (gl.cup_hub_header_ornament_uploaded) {
    ...
    for (uint32_t i = 0; i < 4u; i++)
      glDrawArrays(GL_TRIANGLES, ui->ornament[i] * 6, 6);
    glBindTexture(GL_TEXTURE_2D, gl.tex);
    glUniform1f(gl.loc_image, 0.0f);
    glUniform1f(gl.loc_solid, 1.0f);
  }
  glUniform4f(gl.loc_color, 0.08f, 0.30f, 0.70f, 0.98f);
  if (ui->connector_count)
    glDrawArrays(GL_TRIANGLES, ui->connector_first * 6,
                 ui->connector_count * 6);
  for (uint32_t i = 0; i < ui->row_count; i++) {
    use_rounded_rect(ui->complete ? &ui->champion_style :
                     ui->parent[0] ? &ui->knockout_row_style :
                     &ui->row_style);
    const int selected = ui->editing &&
        competition_frontend_league_table_page() * 5u + i ==
            competition_frontend_league_team_slot_focus();
    glUniform4f(gl.loc_color,
        selected ? 0.99f : ui->complete ? 0.99f : 0.84f,
        selected ? 0.87f : ui->complete ? 0.96f : 0.92f,
        selected ? 0.08f : ui->complete ? 0.82f : 0.99f, 0.99f);
    glDrawArrays(GL_TRIANGLES, ui->rows[i] * 6, 6);
  }
  if (ui->owner_badge_count) {
    use_rounded_rect(&ui->owner_style);
    glUniform4f(gl.loc_color, 0.035f, 0.17f, 0.44f, 0.99f);
    for (uint32_t i = 0; i < ui->row_count; i++)
      if (ui->owner_badge[i])
        glDrawArrays(GL_TRIANGLES, ui->owner_badge[i] * 6, 6);
  }
  use_rounded_rect(&ui->parent_style);
  glUniform4f(gl.loc_color, 0.86f, 0.93f, 1.0f, 0.99f);
  for (uint32_t i = 0; i < 2u; i++)
    if (ui->parent[i]) glDrawArrays(GL_TRIANGLES, ui->parent[i] * 6, 6);
  use_rounded_rect(&ui->schedule_style);
  for (uint32_t i = 0; i < 4u; i++) {
    if (!ui->schedule[i]) continue;
    glUniform4f(gl.loc_color, (i & 1u) ? 0.88f : 0.83f,
        (i & 1u) ? 0.95f : 0.92f, 1.0f, 0.98f);
    glDrawArrays(GL_TRIANGLES, ui->schedule[i] * 6, 6);
  }
  use_rounded_rect(&ui->frame_style);
  for (uint32_t i = 0; i < ui->action_count; i++) {
  ...
  if (ui->trophy) {
    glUniform1f(gl.loc_solid, 0.0f);
    glUniform1f(gl.loc_image, 1.0f);
    glUniform4f(gl.loc_color, 1.0f, 1.0f, 1.0f, 0.96f);
    glBindTexture(GL_TEXTURE_2D, gl.cup_hub_trophy_tex);
    glDrawArrays(GL_TRIANGLES, ui->trophy * 6, 6);
  }
  glUniform1f(gl.loc_solid, 0.0f);
  glUniform1f(gl.loc_image, 1.0f);
  glUniform4f(gl.loc_color, 1.0f, 1.0f, 1.0f, 1.0f);
  glBindTexture(GL_TEXTURE_2D, gl.badge_tex);
  if (ui->badge_count)
    glDrawArrays(GL_TRIANGLES, ui->badge_first * 6,
                 ui->badge_count * 6);
  for (uint32_t i = 0; i < ui->scorer_count; i++) {
    const GLuint portrait = gameplan_portrait_texture(
        ui->scorer_portrait_id[i]);
    if (!portrait) continue;
    glBindTexture(GL_TEXTURE_2D, portrait);
    glDrawArrays(GL_TRIANGLES, ui->scorer_portrait_quad[i] * 6, 6);
  }
  glUniform1f(gl.loc_image, 0.0f);
  glBindTexture(GL_TEXTURE_2D, gl.efootball_tex);
  glUniform4f(gl.loc_color, 0.99f, 1.0f, 1.0f, 1.0f);
  glDrawArrays(GL_TRIANGLES, ui->title_first * 6,
               ui->title_count * 6);
  glDrawArrays(GL_TRIANGLES, ui->bar_first * 6,
               ui->bar_count * 6);
  glUniform4f(gl.loc_color, 0.03f, 0.09f, 0.17f, 1.0f);
  glDrawArrays(GL_TRIANGLES, ui->body_first * 6,
               ui->body_count * 6);
  glUniform4f(gl.loc_color, 1.0f, 0.90f, 0.12f, 1.0f);
  ...
  glDrawArrays(GL_TRIANGLES, ui->helper_first * 6,
               ui->helper_count * 6);
  glBindTexture(GL_TEXTURE_2D, gl.tex);
  glUniform1f(gl.loc_image, 0.0f);
  glUniform1f(gl.loc_solid, 1.0f);
...
static void overlay_render(void) {
  ...
  const int custom_hub_settings_popup = cup_settings_popup || cup_general_popup ||
  ...
  const int custom_competition = competition_frontend_active() &&
                                 !cup_settings_popup && !cup_general_popup &&
                                 !cup_team_picker;
  ...
  static u64 cup_champion_presentation_tick;
  const CompetitionFrontendState competition_current_state =
      competition_frontend_state();
  if (custom_competition &&
      competition_current_state == COMPETITION_FRONTEND_CUP_BRACKET) {
    if (competition_frontend_cup_take_champion_presentation())
      cup_champion_presentation_tick = armGetSystemTick();
  } else {
    cup_champion_presentation_tick = 0;
  }
  ...
  int competition_cup_logo_quad = 0;
  int cup_left_quad = 0, cup_right_quad = 0;
  int cup_left_bar_quad = 0, cup_right_bar_quad = 0;
  int cup_watermark_quad = 0;
  ...
  int cup_champion_quad = 0;
  int cup_champion_ribbon_quad = 0;
  int cup_champion_trophy_quad = 0;
  float cup_champion_trophy_alpha = 0.88f;
  int cup_confetti_gold_first = 0, cup_confetti_gold_count = 0;
  int cup_confetti_blue_first = 0, cup_confetti_blue_count = 0;
  float cup_confetti_alpha = 0.0f;
  int cup_third_quads[2] = {0};
  int cup_history_quads[4] = {0};
  int cup_score_first = 0, cup_score_count = 0;
  int cup_header_ornament_quads[4] = {0};
  int cup_shade_quads[6] = {0};
  int cup_action_frame_quads[5] = {0};
  int cup_action_gloss_quads[5] = {0};
  int cup_action_quads[5] = {0};
  int cup_edit_fixture = -1, cup_edit_side = -1;
  int cup_swap_fixture = -1, cup_swap_side = -1;
  int cup_focus_fixture = -1;
  int cup_connector_first = 0, cup_connector_count = 0;
  int cup_badge_first = 0, cup_badge_count = 0;
  int cup_title_first = 0, cup_title_count = 0;
  int cup_text_first = 0, cup_text_count = 0;
  int cup_champion_ribbon_text_first = 0;
  int cup_champion_ribbon_text_count = 0;
  int cup_action_text_first[5] = {0};
  int cup_action_text_count[5] = {0};
  int cup_rule_scrim_quad = 0, cup_rule_card_quad = 0;
  int cup_rule_header_quad = 0, cup_rule_ok_quad = 0;
  int cup_rule_title_first = 0, cup_rule_title_count = 0;
  int cup_rule_body_first = 0, cup_rule_body_count = 0;
  int cup_rule_ok_text_first = 0, cup_rule_ok_text_count = 0;
  RoundedRectStyle cup_shell_style = {0};
  RoundedRectStyle cup_champion_style = {0};
  RoundedRectStyle cup_third_style = {0};
  RoundedRectStyle cup_header_style = {0};
  RoundedRectStyle cup_left_style = {0};
  RoundedRectStyle cup_right_style = {0};
  RoundedRectStyle cup_left_bar_style = {0};
  RoundedRectStyle cup_right_bar_style = {0};
  ...
  RoundedRectStyle cup_action_frame_style = {0};
  RoundedRectStyle cup_rule_card_style = {0};
  RoundedRectStyle cup_rule_ok_style = {0};
  RoundedRectStyle cup_score_style = {0};
...
#define ADD_SWITCH_HELPER(KEY, CENTER_X, CENTER_Y, SIZE)                 \
  ...
  if (custom_gameplan) {
    ...
    if (competition_display_state == COMPETITION_FRONTEND_LEAGUE_HUB) {
      ...
      if (league_hub_ui.shoulder_count == 2u) {
        ADD_SWITCH_HELPER("SL", league_hub_ui.shoulder_centers[0],
            league_hub_ui.shoulder_y, league_hub_ui.shoulder_size);
        ADD_SWITCH_HELPER("SR", league_hub_ui.shoulder_centers[1],
            league_hub_ui.shoulder_y, league_hub_ui.shoulder_size);
      }
      for (uint32_t i = 0; i < league_hub_ui.scorer_count; i++)
        if (!gameplan_portrait_texture(
                league_hub_ui.scorer_portrait_id[i]))
          pes_controller_league_request_scorer_portrait(
              i, league_hub_ui.scorer_portrait_id[i]);
      for (uint32_t i = 0; i < league_hub_ui.helper_count_keys; i++)
        ADD_SWITCH_HELPER(league_hub_ui.helper_keys[i],
            league_hub_ui.helper_centers[i], league_hub_ui.helper_y,
            league_hub_ui.helper_size);
    } else {
    /*
    ...
    const int competition_settings_page =
        ...
        state == COMPETITION_FRONTEND_LEAGUE_SETTINGS;
    const int league_settings_page =
        state == COMPETITION_FRONTEND_LEAGUE_SETTINGS;
    ...
    const float panel_y = bracket_page
                              ...
                              : league_settings_page
                                    ? 0.130f * (float)screen_height
                              : competition_settings_page
    ...
    if (bracket_page) {
      ...
      const int editing_bracket = competition_frontend_cup_bracket_editing();
      const uint32_t view_round = competition_frontend_cup_view_round();
      const int one_round_final = cup && cup->round_count == 1u;
      const int champion_page = cup &&
          view_round == (one_round_final ? 1u : cup->round_count - 1u);
      const int compact_bracket = cup &&
          !one_round_final && view_round + 2u == cup->round_count;
      const uint32_t schedule_view_round =
          champion_page && one_round_final ? 0u : view_round;
      const uint32_t first = competition_frontend_cup_view_first_fixture();
      const uint32_t fixture_count = cup
          ? cup_tournament_fixture_count(cup, schedule_view_round) : 0u;
      const uint32_t visible = !champion_page && fixture_count > first
          ? (fixture_count - first < 2u ? fixture_count - first : 2u) : 0u;
      ...
      /* Two fixtures per page leave room for wider team cards while the
       * schedule gets a real six-column row instead of cramped crests. */
      const float left_w = (inner_w - gap) * 0.58f;
      ...
      const float card_step = 0.190f * (float)screen_height;
      const float card_h = 0.165f * (float)screen_height;
      const float card_y = one_round_final
          ? body_y + (body_h - card_h) * 0.5f
          : body_y + 0.115f * (float)screen_height;
      const float child_slot_h = card_h * 0.47f;
      const float child_x = inner_x + 0.012f * (float)screen_width;
      const float slot_glyph_h = roundf(fminf(child_slot_h * 0.58f,
                                              (float)screen_height / 31.0f));
      /* Leave a real gap between the longest three-letter owner label and
       * the score plate. The two columns still fit inside the narrower panel. */
      const float slot_label_w = measure_efootball_line(
          "MUN - COM", 9, slot_glyph_h, EFOOTBALL_FONT_BOLD);
      const float child_w = fminf(
          0.205f * (float)screen_width,
          fmaxf(0.190f * (float)screen_width,
              0.068f * (float)screen_height + slot_label_w +
              0.070f * (float)screen_width));
      const float parent_w = child_w;
      const float column_margin = 0.012f * (float)screen_width;
      const float parent_x = compact_bracket
          ? child_x + child_w + 0.032f * (float)screen_width
          : inner_x + left_w - parent_w - column_margin;
      const float parent_h = card_h;
      const float parent_slot_h = parent_h * 0.48f;
      const float champion_w = left_w * 0.33f;
      const float champion_h = body_h * 0.56f;
      const float champion_x = inner_x + left_w * 0.275f;
      const float champion_y = body_y + (body_h - champion_h) * 0.5f;
      const float champion_ribbon_h = 0.056f * (float)screen_height;
      const float third_slot_h = 0.044f * (float)screen_height;
      const float third_y = body_y + body_h - 0.105f * screen_height;
      const float hist_x = right_x + 0.012f * (float)screen_width;
      const float hist_w = right_w - 0.024f * (float)screen_width;
      float history_y = body_y + 0.113f * (float)screen_height;
      const float history_h = 0.092f * (float)screen_height;
      const float history_step = 0.105f * (float)screen_height;
      const float action_y = body_y + body_h +
                             0.060f * (float)screen_height;
      const float action_w = 0.162f * (float)screen_width;
      const float action_h = 0.075f * (float)screen_height;
      const float action_gap = 0.011f * (float)screen_width;
      const uint32_t action_count = cup && cup->champion ? 1u : 4u;
      const float action_x = ((float)screen_width -
          action_count * action_w - (action_count - 1u) * action_gap) * 0.5f;
      float action_positions[5] = {0};
      if (editing_bracket && draft) {
        uint32_t focused_fixture = 0, focused_side = 0;
        if (competition_draft_slot_fixture(
                draft, competition_frontend_cup_bracket_slot_focus(),
                &focused_fixture, &focused_side) &&
            focused_fixture >= first && focused_fixture < first + visible &&
            !view_round) {
          cup_edit_fixture = (int)(focused_fixture - first);
          cup_edit_side = (int)focused_side;
        }
        const uint32_t swap_source =
            competition_frontend_cup_bracket_swap_source();
        if (swap_source < draft->team_count &&
            competition_draft_slot_fixture(draft, swap_source,
                                           &focused_fixture, &focused_side) &&
            focused_fixture >= first && focused_fixture < first + visible &&
            !view_round) {
          cup_swap_fixture = (int)(focused_fixture - first);
          cup_swap_side = (int)focused_side;
        }
      }
      ...
      quads += emit_round_rect_quad(right_x, body_y, right_w, body_h,
                                    verts + quads * 24);
      cup_left_bar_style = (RoundedRectStyle){left_w - 0.016f * screen_width,
          0.053f * screen_height, 0.010f * screen_height};
      cup_left_bar_quad = quads;
      quads += emit_round_rect_quad(inner_x + 0.008f * screen_width,
          body_y + 0.010f * screen_height,
          left_w - 0.016f * screen_width, 0.053f * screen_height,
          verts + quads * 24);
      cup_right_bar_quad = quads;
      cup_right_bar_style = (RoundedRectStyle){right_w - 0.016f * screen_width,
          0.053f * screen_height, 0.010f * screen_height};
      quads += emit_round_rect_quad(right_x + 0.008f * screen_width,
          body_y + 0.010f * screen_height,
          right_w - 0.016f * screen_width, 0.053f * screen_height,
          verts + quads * 24);
      /* Three translucent bands give the light panels a blue depth without
       * baking a second copy of the generated stadium artwork. */
      for (uint32_t band = 0; band < 3u; band++) {
        const float shade_y = body_y + 0.065f * screen_height +
                              band * 0.075f * screen_height;
        cup_shade_quads[band * 2u] = quads;
        quads += emit_rect(inner_x + 0.002f * screen_width, shade_y,
                           left_w - 0.004f * screen_width,
                           0.075f * screen_height, verts + quads * 24);
        cup_shade_quads[band * 2u + 1u] = quads;
        quads += emit_rect(right_x + 0.002f * screen_width, shade_y,
                           right_w - 0.004f * screen_width,
                           0.075f * screen_height, verts + quads * 24);
      }
      const float ornament_w = 0.039f * (float)screen_width;
      const float ornament_h = 0.040f * (float)screen_height;
      const float ornament_y = body_y + 0.016f * (float)screen_height;
      const float ornament_panel_x[2] = {
          inner_x + 0.008f * screen_width,
          right_x + 0.008f * screen_width};
      const float ornament_panel_w[2] = {
          left_w - 0.016f * screen_width,
          right_w - 0.016f * screen_width};
      for (uint32_t panel = 0; panel < 2u; panel++) {
        const uint32_t idx = panel * 2u;
        /* The sprite is one complete motif, not two cropped ends of a banner.
         * Bracket shoulders retain their own clear space at the bar edges. */
        const float inset = (panel ? 0.006f : 0.055f) * screen_width;
        cup_header_ornament_quads[idx] = quads;
        quads += emit_image_rect_uv(
            ornament_panel_x[panel] + inset,
            ornament_y, ornament_w, ornament_h,
            0.0f, 0.0f, 1.0f, 1.0f, verts + quads * 24);
        cup_header_ornament_quads[idx + 1u] = quads;
        quads += emit_image_rect_uv(
            ornament_panel_x[panel] + ornament_panel_w[panel] -
                ornament_w - inset,
            ornament_y, ornament_w, ornament_h,
            1.0f, 0.0f, 0.0f, 1.0f, verts + quads * 24);
      }
      if (!champion_page) {
        const int has_cup_logo = current_cup_logo_texture() != 0u;
        const float mark_w = has_cup_logo
            ? 0.300f * screen_height : 0.225f * screen_width;
        const float mark_h = has_cup_logo
            ? mark_w : mark_w * (496.0f / 1310.0f);
        cup_watermark_quad = quads;
        quads += emit_image_rect(
            inner_x + (left_w - mark_w) * 0.5f,
            body_y + (body_h - mark_h) * 0.5f,
            mark_w, mark_h, verts + quads * 24);
      }
      ...
      for (uint32_t i = 0; i < visible; i++) {
        ...
        if (cup_fixture_is_bye(cup, view_round, first + i, fixture)) {
      ...
      const uint32_t parent_count = one_round_final ? 0u
          : view_round + 1u < cup->round_count
          ? (visible + 1u) / 2u : (visible ? 1u : 0u);
      const CupFixture *schedule[4] = {0};
      uint32_t schedule_round[4] = {0};
      uint32_t schedule_index[4] = {0};
      uint32_t schedule_count = 0;
      for (uint32_t i = 0; i < visible && schedule_count < 4u; i++) {
        const CupFixture *fixture = cup_tournament_fixture(
            cup, view_round, first + i);
        if (fixture && !cup_fixture_is_bye(cup, view_round, first + i,
                                           fixture)) {
          schedule[schedule_count] = fixture;
          schedule_round[schedule_count] = view_round;
          schedule_index[schedule_count++] = first + i;
        }
      }
      for (uint32_t offset = 0; offset < fixture_count &&
                               schedule_count < 4u; offset++) {
        const uint32_t i = (first + visible + offset) % fixture_count;
        if (i >= first && i < first + visible) continue;
        const CupFixture *fixture = cup_tournament_fixture(
            cup, schedule_view_round, i);
        if (fixture && !cup_fixture_is_bye(cup, schedule_view_round, i,
                                           fixture)) {
          schedule[schedule_count] = fixture;
          schedule_round[schedule_count] = schedule_view_round;
          schedule_index[schedule_count++] = i;
        }
      }
      if (view_round + 1u < cup->round_count) {
        for (uint32_t i = 0; i < parent_count && schedule_count < 4u; i++) {
          schedule[schedule_count] = cup_tournament_fixture(
              cup, view_round + 1u, first / 2u + i);
          schedule_round[schedule_count] = view_round + 1u;
          schedule_index[schedule_count++] = first / 2u + i;
        }
      }
      const CupFixture *bronze = cup_tournament_third_place_fixture(cup);
      if (bronze && schedule_count < 4u &&
          view_round + 2u >= cup->round_count) {
        schedule[schedule_count] = bronze;
        schedule_round[schedule_count] = cup->round_count - 1u;
        schedule_index[schedule_count++] = CUP_THIRD_PLACE_INDEX;
      }
      uint32_t schedule_teams[4][2] = {{0}};
      for (uint32_t i = 0; i < schedule_count; i++)
        cup_tournament_schedule_teams(cup, schedule_round[i],
                                      schedule_index[i],
                                      &schedule_teams[i][0],
                                      &schedule_teams[i][1]);
      if (schedule_count) {
        const float content_top = body_y + 0.077f * screen_height;
        const float content_bottom = body_y + body_h -
                                     0.020f * screen_height;
        const float schedule_h = history_h +
            history_step * (float)(schedule_count - 1u);
        history_y = content_top +
            fmaxf(0.0f, content_bottom - content_top - schedule_h) * 0.5f;
      }
      ...
      if (champion_page) {
        cup_champion_style = (RoundedRectStyle){
            champion_w, champion_h, 0.012f * screen_height};
        cup_champion_quad = quads;
        quads += emit_round_rect_quad(
            champion_x, champion_y, champion_w, champion_h,
            verts + quads * 24);
        cup_champion_ribbon_quad = quads;
        quads += emit_rect(champion_x,
            champion_y + champion_h - champion_ribbon_h,
            champion_w, champion_ribbon_h, verts + quads * 24);
        cup_champion_trophy_quad = quads;
        float lift = 1.0f;
        float presentation_seconds = 9.0f;
        if (cup_champion_presentation_tick) {
          presentation_seconds = (float)armTicksToNs(
              armGetSystemTick() - cup_champion_presentation_tick) /
              1000000000.0f;
          lift = fminf(1.0f, presentation_seconds / 1.15f);
          lift = 1.0f - (1.0f - lift) * (1.0f - lift) *
                          (1.0f - lift);
          cup_champion_trophy_alpha = 0.30f + 0.58f * lift;
        }
        const float trophy_scale = 0.72f + 0.28f * lift;
        const float trophy_w = 0.105f * screen_width * trophy_scale;
        const float trophy_h = 0.235f * screen_height * trophy_scale;
        const float trophy_center_x = champion_x + champion_w +
            0.026f * screen_width + 0.105f * screen_width * 0.5f;
        const float trophy_center_y = champion_y + champion_h * 0.5f;
        quads += emit_image_rect(
            trophy_center_x - trophy_w * 0.5f,
            trophy_center_y - trophy_h * 0.5f +
                (1.0f - lift) * 0.16f * screen_height,
            trophy_w, trophy_h, verts + quads * 24);
        /* A small, bounded burst accompanies the one-shot 2D lift after a
         * human wins the final. It reuses the existing vertex batch and never
         * decodes or uploads a texture per frame. */
        if (presentation_seconds >= 0.75f &&
            presentation_seconds < 2.75f) {
          const float burst = (presentation_seconds - 0.75f) / 2.0f;
          cup_confetti_alpha = 0.82f * (1.0f - burst);
          for (uint32_t color = 0; color < 2u; color++) {
            int *first_quad = color ? &cup_confetti_blue_first
                                    : &cup_confetti_gold_first;
            int *quad_count = color ? &cup_confetti_blue_count
                                    : &cup_confetti_gold_count;
            *first_quad = quads;
            for (uint32_t i = 0; i < 7u; i++) {
              const float side = (float)i - 3.0f + (color ? 0.5f : 0.0f);
              const float x = trophy_center_x + side *
                  (0.014f + burst * 0.020f) * screen_width;
              const float y = trophy_center_y -
                  (0.025f + (float)((i * 3u + color) % 5u) * 0.013f) *
                      screen_height - burst * 0.105f * screen_height +
                  burst * burst * 0.145f * screen_height;
              quads += emit_rect(x, y, 0.003f * screen_width,
                  0.010f * screen_height, verts + quads * 24);
              (*quad_count)++;
            }
          }
        }
      }
      if (compact_bracket && cup->third_place_enabled) {
        cup_third_style = (RoundedRectStyle){parent_w, third_slot_h,
                                              0.006f * screen_height};
        for (uint32_t side = 0; side < 2u; side++) {
          cup_third_quads[side] = quads;
          quads += emit_round_rect_quad(parent_x,
              third_y + side * (third_slot_h + 0.003f * screen_height),
              parent_w, third_slot_h, verts + quads * 24);
        }
      }
      cup_border_first = quads;
      const float border = fmaxf(1.8f, (float)screen_height / 420.0f);
      for (uint32_t i = 0; i < visible; i++) {
      ...
      if (champion_page) {
        const float y = champion_y;
        quads += emit_rect(champion_x, y, champion_w, border,
                           verts + quads * 24);
        quads += emit_rect(champion_x, y + champion_h - border,
                           champion_w, border, verts + quads * 24);
        quads += emit_rect(champion_x, y, border, champion_h,
                           verts + quads * 24);
        quads += emit_rect(champion_x + champion_w - border, y,
                           border, champion_h, verts + quads * 24);
        cup_border_count += 4;
      }
      if (compact_bracket && cup->third_place_enabled) {
        for (uint32_t side = 0; side < 2u; side++) {
          const float y3 = third_y + side *
              (third_slot_h + 0.003f * screen_height);
          quads += emit_rect(parent_x, y3, parent_w, border,
                             verts + quads * 24);
          quads += emit_rect(parent_x, y3 + third_slot_h - border,
                             parent_w, border, verts + quads * 24);
          quads += emit_rect(parent_x, y3, border, third_slot_h,
                             verts + quads * 24);
          quads += emit_rect(parent_x + parent_w - border, y3,
                             border, third_slot_h, verts + quads * 24);
          cup_border_count += 4;
        }
      }
      cup_connector_first = quads;
      if (one_round_final && !champion_page && visible) {
        const float line_start = child_x + child_w;
        const float line_end = inner_x + left_w - 0.005f * screen_width;
        if (line_end > line_start) {
          quads += emit_rect(line_start, card_y + card_h * 0.5f,
              line_end - line_start,
              fmaxf(2.8f, screen_height / 250.0f), verts + quads * 24);
          cup_connector_count++;
        }
      }
      for (uint32_t i = 0; i < parent_count && i < 4u; i++) {
        ...
        const float thick = fmaxf(2.8f, screen_height / 250.0f);
        if (view_round + 1u == cup->round_count) {
      ...
      if (compact_bracket && parent_count) {
        /* Final feeds the separate Champion page; keep the link visible at
         * this page's right edge even though the champion card is elsewhere. */
        const float line_start = parent_x + parent_w;
        const float join_x = line_start + 0.008f * screen_width;
        const float line_end = inner_x + left_w - 0.005f * screen_width;
        if (line_end > join_x) {
          const float final_y = card_y + card_step * 0.5f;
          const float top_center = final_y + parent_slot_h * 0.5f;
          const float bottom_center = final_y + parent_h -
                                      parent_slot_h * 0.5f;
          const float line_thick = fmaxf(2.8f, screen_height / 250.0f);
          quads += emit_rect(line_start, top_center,
              join_x - line_start, line_thick, verts + quads * 24);
          cup_connector_count++;
          quads += emit_rect(line_start, bottom_center,
              join_x - line_start, line_thick, verts + quads * 24);
          cup_connector_count++;
          quads += emit_rect(join_x, top_center,
              line_thick, bottom_center - top_center + line_thick,
              verts + quads * 24);
          cup_connector_count++;
          quads += emit_rect(join_x,
              (top_center + bottom_center) * 0.5f,
              line_end - join_x, line_thick, verts + quads * 24);
          cup_connector_count++;
        }
      }
      if (champion_page) {
        quads += emit_rect(child_x,
            champion_y + champion_h * 0.5f,
            champion_x - child_x,
            fmaxf(2.8f, screen_height / 250.0f),
            verts + quads * 24);
        cup_connector_count++;
      }
      cup_score_style = (RoundedRectStyle){0.038f * screen_width,
          child_slot_h * 0.82f, 0.006f * screen_height};
      cup_score_first = quads;
      for (uint32_t i = 0; i < visible; i++) {
        if (!cup_fixture_quads[i]) continue;
        for (uint32_t side = 0; side < 2u; side++) {
          const float slot_y = card_y + card_step * i +
              (side ? card_h - child_slot_h : 0.0f);
          quads += emit_round_rect_quad(
              child_x + child_w - 0.003f * screen_width -
                  cup_score_style.width,
              slot_y + (child_slot_h - cup_score_style.height) * 0.5f,
              cup_score_style.width, cup_score_style.height,
              verts + quads * 24);
          cup_score_count++;
        }
      }
      for (uint32_t i = 0; i < parent_count && i < 4u; i++) {
        if (!cup_next_away_quads[i]) continue;
        const float y = card_y + card_step * (2u * i) +
                        card_step * 0.5f;
        for (uint32_t side = 0; side < 2u; side++) {
          const float slot_y = y +
              (side ? parent_h - parent_slot_h : 0.0f);
          quads += emit_round_rect_quad(
              parent_x + parent_w - 0.003f * screen_width -
                  cup_score_style.width,
              slot_y + (parent_slot_h - cup_score_style.height) * 0.5f,
              cup_score_style.width, cup_score_style.height,
              verts + quads * 24);
          cup_score_count++;
        }
      ...
      const float action_frame = fmaxf(2.0f, screen_height / 320.0f);
      cup_action_frame_style = (RoundedRectStyle){
          action_w + action_frame * 2.0f,
          action_h + action_frame * 2.0f,
          0.018f * screen_height};
      uint32_t visible_action = 0;
      for (uint32_t i = 0; i < action_count; i++) {
        action_positions[i] = action_x +
            (action_w + action_gap) * (float)visible_action++;
        cup_action_frame_quads[i] = quads;
        quads += emit_round_rect_quad(
            action_positions[i] - action_frame, action_y - action_frame,
            cup_action_frame_style.width, cup_action_frame_style.height,
            verts + quads * 24);
        cup_action_quads[i] = quads;
        quads += emit_round_rect_quad(action_positions[i], action_y,
                                      action_w, action_h,
                                      verts + quads * 24);
        cup_action_gloss_quads[i] = quads;
        quads += emit_rect(action_positions[i] + 0.006f * screen_width,
                           action_y + 0.007f * screen_height,
                           action_w - 0.012f * screen_width,
                           0.017f * screen_height, verts + quads * 24);
      }
      ...
      for (uint32_t i = 0; i < visible; i++) {
        const CupFixture *fixture = cup_tournament_fixture(
            cup, view_round, first + i);
        const float y = card_y + card_step * (float)i;
        const int bye = cup_fixture_is_bye(cup, view_round, first + i,
                                           fixture);
        const float badge = 0.068f * (float)screen_height;
      ...
      if (champion_page && cup->champion) {
        const float badge = fminf(champion_w * 0.53f,
                                  champion_h * 0.52f);
        quads += emit_badge(
            exhibition_team_catalog_badge(cup->champion),
            champion_x + (champion_w - badge) * 0.5f,
            champion_y + 0.015f * screen_height,
            badge, badge, verts + quads * 24);
        cup_badge_count++;
      }
      if (compact_bracket) {
        const CupFixture *third = cup_tournament_third_place_fixture(cup);
        if (third) {
          const uint32_t teams[2] = {third->home, third->away};
          for (uint32_t side = 0; side < 2u; side++) {
            if (!teams[side]) continue;
            const float badge = third_slot_h * 0.86f;
            quads += emit_badge(exhibition_team_catalog_badge(teams[side]),
                parent_x + 0.008f * screen_width,
                third_y + side * (third_slot_h + 0.003f * screen_height) +
                    (third_slot_h - badge) * 0.5f,
                badge, badge, verts + quads * 24);
            cup_badge_count++;
          }
        }
      }
      const float history_badge = 0.068f * (float)screen_height;
      for (uint32_t i = 0; i < schedule_count; i++) {
        const uint32_t *teams = schedule_teams[i];
        const float y = history_y + history_step * (float)i;
        for (uint32_t side = 0; side < 2u; side++) {
          if (!teams[side]) continue;
          quads += emit_badge(exhibition_team_catalog_badge(teams[side]),
              cup_schedule_badge_x(hist_x, hist_w, side != 0u),
      ...
      const float head_size = roundf((float)screen_height / 36.0f);
      const char *cup_name = competition_frontend_cup_name();
      cup_title_first = quads;
      quads += emit_efootball_centered_fit_line(
          ...
          verts + quads * 24);
      const char *bracket_title = champion_page ? "CHAMPION" : "BRACKET";
      quads += emit_efootball_centered_fit_line(
          bracket_title, (int)strlen(bracket_title),
          inner_x + left_w * 0.5f,
          body_y + 0.010f * screen_height +
              (0.053f * screen_height - head_size) * 0.5f,
          left_w * 0.72f, head_size, head_size * 0.86f,
          EFOOTBALL_FONT_STENCIL, verts + quads * 24);
      const char *right_title = "MATCH SCHEDULE";
      quads += emit_efootball_centered_fit_line(
          right_title, (int)strlen(right_title), right_x + right_w * 0.5f,
          body_y + 0.010f * screen_height +
              (0.053f * screen_height - head_size) * 0.5f,
          right_w * 0.90f, head_size, head_size * 0.82f,
          EFOOTBALL_FONT_STENCIL, verts + quads * 24);
      cup_title_count = quads - cup_title_first;
      cup_text_first = quads;
      if (!champion_page) {
        const char *round_name =
            competition_frontend_cup_round_name(view_round);
        char round_heading[64];
        const uint32_t pages_this_round =
            competition_frontend_cup_view_page_count();
        if (compact_bracket || pages_this_round <= 1u)
          snprintf(round_heading, sizeof(round_heading), "%s", round_name);
        else
          snprintf(round_heading, sizeof(round_heading), "%s  %u / %u",
                   round_name, first / 2u + 1u, pages_this_round);
        quads += emit_efootball_fit_line(
            round_heading, (int)strlen(round_heading), child_x,
            body_y + 0.077f * (float)screen_height, child_w,
            head_size, head_size * 0.60f, EFOOTBALL_FONT_STENCIL,
            verts + quads * 24);
        if (!one_round_final) {
          const char *advance_label =
              competition_frontend_cup_round_name(view_round + 1u);
          quads += emit_efootball_fit_line(
              advance_label, (int)strlen(advance_label), parent_x,
              body_y + 0.077f * (float)screen_height, parent_w,
              head_size, head_size * 0.60f, EFOOTBALL_FONT_STENCIL,
              verts + quads * 24);
        }
      }
      if (compact_bracket) {
        if (cup->third_place_enabled) {
          const char *third_heading = "3RD PLACE";
          quads += emit_efootball_fit_line(
              third_heading, 9, parent_x,
              third_y - 0.036f * screen_height, parent_w,
              head_size, head_size * 0.60f,
              EFOOTBALL_FONT_STENCIL, verts + quads * 24);
        }
      }
      for (uint32_t i = 0; i < visible; i++) {
        ...
        for (uint32_t side = 0; side < sides; side++) {
          const float slot_y = y + (bye
              ...
              : side ? card_h - child_slot_h : 0.0f);
          const uint32_t visual_slot = draft && !view_round
              ? competition_draft_fixture_slot(draft, first + i, side)
              : UINT32_MAX;
          const uint32_t owner_hint = draft && visual_slot < draft->team_count
              ? draft->owners[visual_slot] : UINT32_MAX;
          quads += emit_cup_slot_text(
              teams[side], owner_hint,
              !bye,
              (fixture->complete || fixture->first_leg_complete) &&
                  fixture->home && fixture->away,
              fixture->complete
                  ? (side ? fixture->away_goals : fixture->home_goals)
                  : (side ? fixture->first_leg_away_goals
      ...
      for (uint32_t i = 0; i < parent_count && i < 4u; i++) {
        ...
        if (final) {
          quads += emit_cup_slot_text(
              fixture ? fixture->winner : 0u, UINT32_MAX, 0, 0, 0u,
        ...
        for (uint32_t side = 0; side < 2u; side++) {
          quads += emit_cup_slot_text(
              teams[side], UINT32_MAX,
              1,
              fixture && fixture->complete && fixture->home && fixture->away,
      ...
      if (champion_page) {
        const char *winner_name = cup->champion
            ? exhibition_team_catalog_name(cup->champion) : "TBD";
        if (!winner_name || !winner_name[0]) winner_name = "TBD";
        quads += emit_efootball_centered_fit_line(
            winner_name, (int)strlen(winner_name),
            champion_x + champion_w * 0.5f,
            champion_y + champion_h - champion_ribbon_h -
                0.045f * screen_height,
            champion_w * 0.90f, head_size * 0.92f,
            head_size * 0.60f, EFOOTBALL_FONT_BOLD,
            verts + quads * 24);
        cup_champion_ribbon_text_first = quads;
        quads += emit_efootball_centered_fit_line(
            "CHAMPION", 8, champion_x + champion_w * 0.5f,
            champion_y + champion_h - champion_ribbon_h +
                (champion_ribbon_h - head_size) * 0.5f,
            champion_w * 0.88f, head_size,
            head_size * 0.68f, EFOOTBALL_FONT_BOLD,
            verts + quads * 24);
        cup_champion_ribbon_text_count =
            quads - cup_champion_ribbon_text_first;
      }
      if (compact_bracket) {
        const CupFixture *third = cup_tournament_third_place_fixture(cup);
        if (third) {
          ...
          for (uint32_t side = 0; side < 2u; side++)
            quads += emit_cup_slot_text(
                teams[side], UINT32_MAX, 1,
                third->complete && third->home && third->away,
                side ? third->away_goals : third->home_goals,
                parent_x,
                third_y + side * (third_slot_h + 0.003f * screen_height),
                parent_w, third_slot_h, verts + quads * 24);
        }
      ...
      for (uint32_t i = 0; i < schedule_count; i++) {
        const CupFixture *fixture = schedule[i];
        if (!fixture) continue;
        const float y = history_y + history_step * (float)i;
        quads += emit_cup_schedule_match_text(
            schedule_teams[i][0], schedule_teams[i][1],
            fixture->complete ? fixture->home_goals
                              : fixture->first_leg_home_goals,
            fixture->complete ? fixture->away_goals
                              : fixture->first_leg_away_goals,
            (fixture->complete || fixture->first_leg_complete) &&
                fixture->away,
            hist_x, y, hist_w, history_h,
            verts + quads * 24);
      }
      cup_text_count = quads - cup_text_first;
      const char *const actions[4] = {
          "TEAMS", "NEXT", "GENERAL SETTING", "SAVE"};
      for (uint32_t i = 0; i < action_count; i++) {
        cup_action_text_first[i] = quads;
        quads += emit_efootball_centered_fit_line(
            cup->champion ? "TOP TO MENU" : actions[i],
            (int)strlen(cup->champion ? "TOP TO MENU" : actions[i]),
            action_positions[i] + action_w * 0.5f,
      ...
      if (cup_rule_modal) {
        const float modal_w = 0.56f * screen_width;
        const float modal_h = 0.282f * screen_height;
        const float modal_x = (screen_width - modal_w) * 0.5f;
        const float modal_y = (screen_height - modal_h) * 0.5f;
        const float modal_head_h = 0.064f * screen_height;
        const float ok_w = 0.126f * screen_width;
        const float ok_h = 0.052f * screen_height;
        const float ok_x = (screen_width - ok_w) * 0.5f;
        const float ok_y = modal_y + modal_h - ok_h -
                           0.016f * screen_height;
        const float modal_text_h = roundf(screen_height / 33.0f);
        cup_rule_card_style = (RoundedRectStyle){
            modal_w, modal_h, 0.018f * screen_height};
        cup_rule_ok_style = (RoundedRectStyle){
            ok_w, ok_h, 0.010f * screen_height};
        cup_rule_scrim_quad = quads;
        quads += emit_rect(0.0f, 0.0f, screen_width, screen_height,
                           verts + quads * 24);
        cup_rule_card_quad = quads;
        quads += emit_round_rect_quad(modal_x, modal_y, modal_w, modal_h,
                                      verts + quads * 24);
        cup_rule_header_quad = quads;
        quads += emit_rect(modal_x, modal_y, modal_w, modal_head_h,
                           verts + quads * 24);
        cup_rule_ok_quad = quads;
        quads += emit_round_rect_quad(ok_x, ok_y, ok_w, ok_h,
                                      verts + quads * 24);
        cup_rule_title_first = quads;
        cup_rule_title_count = emit_efootball_centered_fit_line(
            "BRACKET RULE", 12, screen_width * 0.5f,
            modal_y + (modal_head_h - modal_text_h) * 0.5f,
            modal_w * 0.88f, modal_text_h, modal_text_h * 0.78f,
            EFOOTBALL_FONT_STENCIL, verts + quads * 24);
        quads += cup_rule_title_count;
        cup_rule_body_first = quads;
        cup_rule_body_count = emit_efootball_centered_fit_line(
            "AT LEAST ONE PLAYER MUST PLAY", 29,
            screen_width * 0.5f, modal_y + 0.094f * screen_height,
            modal_w * 0.88f, modal_text_h, modal_text_h * 0.74f,
            EFOOTBALL_FONT_BOLD, verts + quads * 24);
        quads += cup_rule_body_count;
        const int second_line = emit_efootball_centered_fit_line(
            "IN THE FIRST ROUND", 18, screen_width * 0.5f,
            modal_y + 0.132f * screen_height,
            modal_w * 0.88f, modal_text_h, modal_text_h * 0.74f,
            EFOOTBALL_FONT_BOLD, verts + quads * 24);
        cup_rule_body_count += second_line;
        quads += second_line;
        cup_rule_ok_text_first = quads;
        cup_rule_ok_text_count = emit_efootball_centered_fit_line(
            "OK", 2, screen_width * 0.5f,
            ok_y + (ok_h - modal_text_h) * 0.5f,
            ok_w * 0.80f, modal_text_h, modal_text_h * 0.75f,
            EFOOTBALL_FONT_BOLD, verts + quads * 24);
        quads += cup_rule_ok_text_count;
      }
      ...
      const uint32_t row_count = league_settings_page ? 4u : 5u;
      const uint32_t visible_rows = row_count;
      const float row_x = panel_x + 0.040f * (float)screen_width;
      const float row_w = panel_w - 0.080f * (float)screen_width;
      const float row_h = (league_settings_page ? 0.103f : 0.063f) *
                          (float)screen_height;
      const float row_step = (league_settings_page ? 0.118f : 0.075f) *
                             (float)screen_height;
      const float value_w = 0.205f * (float)screen_width;
      const float value_h = (league_settings_page ? 0.071f : 0.047f) *
                             (float)screen_height;
      ...
      if (state == COMPETITION_FRONTEND_CUP_SETTINGS) {
        const float logo_area_w = 0.220f * (float)screen_width;
        const float logo_area_h = 0.222f * (float)screen_height;
        ...
        const int has_cup_logo = current_cup_logo_texture() != 0u;
        const float logo_w = has_cup_logo
            ? 0.195f * (float)screen_height : logo_area_w * 0.86f;
        const float logo_h = has_cup_logo
            ? logo_w : logo_w * (496.0f / 1310.0f);
        competition_cup_logo_quad = quads;
        quads += emit_image_rect(logo_area_x + (logo_area_w - logo_w) * 0.5f,
                                logo_area_y + (logo_area_h - logo_h) * 0.5f,
                                logo_w, logo_h, verts + quads * 24);
      }
      ...
      if (competition_team_page && !competition_team_picker_page) {
        ...
        const float label_h = (float)screen_height /
                              (league_settings_page ? 32.0f : 42.0f);
        const float value_gh = (float)screen_height /
                               (league_settings_page ? 34.0f : 43.0f);
        for (uint32_t row = start_row; row < end_row && row < 12u; row++) {
          const uint32_t visible = row - start_row;
    ...
    const int cup_done = bracket_page &&
        competition_frontend_cup_tournament()->champion != 0u;
    const int show_a = state != COMPETITION_FRONTEND_CUP_CHECKPOINT;
    const int edit_helpers = bracket_page &&
                             competition_frontend_cup_bracket_editing();
    const CompetitionEntryDraft *helper_draft = bracket_page
        ? competition_frontend_cup_draft() : NULL;
    const int swapping = edit_helpers &&
        competition_frontend_cup_bracket_swap_source() != UINT32_MAX;
    const int show_x = edit_helpers && !swapping && helper_draft &&
        competition_draft_assigned_count(helper_draft) <
            helper_draft->team_count;
    const int show_y = edit_helpers
        ? !swapping && helper_draft && helper_draft->team_count > 1u
        : bracket_page && !cup_done &&
          competition_frontend_cup_view_page_count() > 1u;
    const char *a_label = swapping ? "PLACE" : cup_done ? "CONFIRM"
        : edit_helpers ? "SELECT TEAM" : "SELECT";
    const char *b_label = swapping ? "CANCEL" : bracket_page && !edit_helpers
        ? "TOP TO MENU" : "BACK";
    const float helper_icon_size = 0.034f * (float)screen_height;
    const float helper_start = bracket_page
        ? panel_x + 0.012f * (float)screen_width
        : panel_x + 0.030f * (float)screen_width;
    const char *helper_keys[4] = {"A", "B", "X", "Y"};
    const char *helper_labels[4] = {
        a_label, b_label, "RANDOM TEAM",
        edit_helpers ? "SWAP" : "NEXT PAGE"};
    const int helper_visible[4] = {show_a, !cup_done, show_x, show_y};
    float helper_label_x[4] = {0};
    float helper_x = helper_start;
    for (uint32_t i = 0; i < 4u; i++) {
      if (!helper_visible[i]) continue;
      ADD_SWITCH_HELPER(helper_keys[i], helper_x + helper_icon_size * 0.5f,
                        helper_y, helper_icon_size);
      helper_label_x[i] = helper_x + helper_icon_size +
                          0.008f * (float)screen_width;
      helper_x = helper_label_x[i] + measure_efootball_line(
          helper_labels[i], (int)strlen(helper_labels[i]), helper_text_gh,
          EFOOTBALL_FONT_BOLD) + 0.021f * (float)screen_width;
    }
    if (bracket_page && !cup_done && !edit_helpers &&
        competition_frontend_cup_view_stage_count() > 1u) {
      const float bracket_inner_x = panel_x + 0.020f * screen_width;
      const float bracket_inner_w = panel_w - 0.040f * screen_width;
      const float bracket_ratio = 0.58f;
      const float bracket_left_w =
          (bracket_inner_w - 0.018f * screen_width) * bracket_ratio;
      const float bracket_bar_y = panel_y + 0.132f * screen_height +
                                  0.037f * screen_height;
      ADD_SWITCH_HELPER("SL", bracket_inner_x + 0.031f * screen_width,
                        bracket_bar_y, helper_icon_size * 1.12f);
      ADD_SWITCH_HELPER("SR", bracket_inner_x + bracket_left_w -
                        0.031f * screen_width, bracket_bar_y,
                        helper_icon_size * 1.12f);
    }
    competition_helper_first_quad = quads;
    for (uint32_t i = 0; i < 4u; i++) {
      if (!helper_visible[i]) continue;
      const int n = emit_efootball_line(
          helper_labels[i], (int)strlen(helper_labels[i]),
          helper_label_x[i], helper_y - helper_text_gh * 0.5f,
          helper_text_gh, EFOOTBALL_FONT_BOLD, verts + quads * 24);
    ...
    for (uint32_t pad = 0; pad < selector_sides; pad++) {
      const float left = panel_x[pad];
      char cup_owner_label[12];
      const char *player = pad == 0 ? "HOME" : "AWAY";
      if (cup_team_picker) {
        const CompetitionEntryDraft *draft =
            competition_frontend_cup_draft();
        const uint32_t slot = competition_frontend_cup_bracket_slot_focus();
        const uint32_t owner = draft && slot < draft->team_count
            ? draft->owners[slot] : 0u;
        if (owner)
          snprintf(cup_owner_label, sizeof(cup_owner_label), "P%u", owner);
        else
          memcpy(cup_owner_label, "COM", 4);
        player = cup_owner_label;
      }
    ...
    const uint32_t item_count = cup_settings_popup
                                ...
                                : cup_general_popup
                                    ? competition_frontend_cup_general_count()
                                : pause_settings_popup
    ...
    const uint32_t focus = cup_settings_popup
                           ...
                           : cup_general_popup
                               ? competition_frontend_cup_general_focus()
                           : pause_settings_popup
    ...
    for (uint32_t slot = 0; slot < visible_count; slot++) {
      ...
      if (cup_settings_popup && !league_settings_popup &&
          ...
          competition_frontend_cup_catalog_index() < FL26_CUP_CUSTOM_INDEX)
        continue; /* AFC, Euro, and World Cup have fixed team counts. */
    ...
    if (focus < item_count && focus_slot < visible_count) {
      const char *label = cup_settings_popup
                          ...
                          : cup_general_popup
                              ? competition_frontend_cup_general_label(focus)
                          : pause_settings_popup
    ...
    for (uint32_t slot = 0; slot < visible_count; slot++) {
      ...
      const char *label = cup_settings_popup
                          ...
                          : cup_general_popup
                              ? competition_frontend_cup_general_label(item)
                          : pause_settings_popup
    ...
    const char *title = league_settings_popup
                            ? "LEAGUE SETTINGS"
                        : cup_settings_popup
                            ? "CUP SETTINGS"
                        : pause_settings_popup
    ...
    for (uint32_t slot = 0; slot < visible_count; slot++) {
      ...
      const char *value = cup_settings_popup
                              ? competition_frontend_item_value(
                                    competition_frontend_cup_setting_row(item))
                          : cup_general_popup
                              ? competition_frontend_cup_general_value(item)
                          : pause_settings_popup
  ...
  if (custom_competition) {
    ...
    const int competition_settings_page =
        ...
        competition_display_state == COMPETITION_FRONTEND_LEAGUE_SETTINGS;
    const int bracket_page =
        competition_display_state == COMPETITION_FRONTEND_CUP_BRACKET ||
        competition_display_state == COMPETITION_FRONTEND_CUP_CHECKPOINT;
    const int competition_team_page =
    ...
    glUniform4f(gl.loc_color, 1.0f, 1.0f, 1.0f, 1.0f);
    glBindTexture(GL_TEXTURE_2D,
                  bracket_page ? gl.cup_hub_stadium_tex
    ...
    if (bracket_page) {
      ...
      glUniform4f(gl.loc_color, 0.94f, 0.97f, 1.0f, 0.97f);
      ...
      for (uint32_t band = 0; band < 3u; band++) {
        glUniform4f(gl.loc_color, 0.52f, 0.70f, 0.96f,
                    0.12f - 0.025f * band);
        glDrawArrays(GL_TRIANGLES, cup_shade_quads[band * 2u] * 6, 12);
      }
      use_rounded_rect(&cup_left_bar_style);
      glUniform4f(gl.loc_color, 0.035f, 0.16f, 0.39f, 0.98f);
      glDrawArrays(GL_TRIANGLES, cup_left_bar_quad * 6, 6);
      use_rounded_rect(&cup_right_bar_style);
      glDrawArrays(GL_TRIANGLES, cup_right_bar_quad * 6, 6);
      use_rounded_rect(NULL);
      glUniform1f(gl.loc_solid, 0.0f);
      glUniform1f(gl.loc_image, 1.0f);
      glUniform4f(gl.loc_color, 1.0f, 1.0f, 1.0f, 0.94f);
      ...
      if (cup_watermark_quad) {
        const GLuint logo_tex = current_cup_logo_texture();
        glUniform4f(gl.loc_color, 1.0f, 1.0f, 1.0f, 0.22f);
        glBindTexture(GL_TEXTURE_2D,
                      logo_tex ? logo_tex : gl.main_menu_brand_tex);
        glDrawArrays(GL_TRIANGLES, cup_watermark_quad * 6, 6);
      }
      if (cup_champion_trophy_quad) {
        glBindTexture(GL_TEXTURE_2D, gl.cup_hub_trophy_tex);
        glUniform4f(gl.loc_color, 1.0f, 1.0f, 1.0f,
                    cup_champion_trophy_alpha);
        glDrawArrays(GL_TRIANGLES, cup_champion_trophy_quad * 6, 6);
      }
      glBindTexture(GL_TEXTURE_2D, gl.tex);
      glUniform1f(gl.loc_image, 0.0f);
      glUniform1f(gl.loc_solid, 1.0f);
      ...
      for (uint32_t i = 0; i < 8u; i++) {
        if (!cup_fixture_quads[i]) continue;
        const int home_edit = (int)i == cup_edit_fixture &&
                              cup_edit_side == 0;
        const int home_swap = (int)i == cup_swap_fixture &&
                              cup_swap_side == 0;
        const int home_next = !competition_frontend_cup_bracket_editing() &&
                              (int)i == cup_focus_fixture;
        glUniform4f(gl.loc_color,
                    home_swap ? 0.99f : home_edit ? 0.99f : home_next ? 0.68f : 0.85f,
                    home_swap ? 0.66f : home_edit ? 0.88f : home_next ? 0.87f : 0.93f,
                    home_swap ? 0.13f : home_edit ? 0.10f : home_next ? 1.0f : 0.99f,
                    0.99f);
        glDrawArrays(GL_TRIANGLES, cup_fixture_quads[i] * 6, 6);
        if (cup_fixture_away_quads[i]) {
          const int away_edit = (int)i == cup_edit_fixture &&
                                cup_edit_side == 1;
          const int away_swap = (int)i == cup_swap_fixture &&
                                cup_swap_side == 1;
          glUniform4f(gl.loc_color,
                      away_swap ? 0.99f : away_edit ? 0.99f : home_next ? 0.68f : 0.85f,
                      away_swap ? 0.66f : away_edit ? 0.88f : home_next ? 0.87f : 0.93f,
                      away_swap ? 0.13f : away_edit ? 0.10f : home_next ? 1.0f : 0.99f,
                      0.99f);
      ...
      for (uint32_t i = 0; i < 8u; i++) {
        if (cup_bye_quads[i]) {
          const int editing = (int)i == cup_edit_fixture;
          const int swapping = (int)i == cup_swap_fixture;
          glUniform4f(gl.loc_color, swapping ? 0.99f : editing ? 0.99f : 0.85f,
                      swapping ? 0.66f : editing ? 0.88f : 0.93f,
                      swapping ? 0.13f : editing ? 0.10f : 0.99f, 0.99f);
      ...
      if (cup_champion_quad) {
        use_rounded_rect(&cup_champion_style);
        glUniform4f(gl.loc_color, 0.97f, 0.96f, 0.87f, 0.99f);
        glDrawArrays(GL_TRIANGLES, cup_champion_quad * 6, 6);
        use_rounded_rect(NULL);
        glUniform4f(gl.loc_color, 0.025f, 0.095f, 0.23f, 0.99f);
        glDrawArrays(GL_TRIANGLES, cup_champion_ribbon_quad * 6, 6);
      }
      if (cup_confetti_gold_count) {
        glUniform4f(gl.loc_color, 1.0f, 0.77f, 0.20f,
                    cup_confetti_alpha);
        glDrawArrays(GL_TRIANGLES, cup_confetti_gold_first * 6,
                     cup_confetti_gold_count * 6);
        glUniform4f(gl.loc_color, 0.18f, 0.49f, 0.88f,
                    cup_confetti_alpha);
        glDrawArrays(GL_TRIANGLES, cup_confetti_blue_first * 6,
                     cup_confetti_blue_count * 6);
      }
      if (cup_third_quads[0]) {
        use_rounded_rect(&cup_third_style);
        for (uint32_t side = 0; side < 2u; side++)
          glDrawArrays(GL_TRIANGLES, cup_third_quads[side] * 6, 6);
      }
      use_rounded_rect(&cup_score_style);
      glUniform4f(gl.loc_color, 0.95f, 0.98f, 1.0f, 0.89f);
      glDrawArrays(GL_TRIANGLES, cup_score_first * 6,
                   cup_score_count * 6);
      use_rounded_rect(NULL);
      glUniform4f(gl.loc_color, 0.16f, 0.37f, 0.67f, 0.97f);
      ...
      for (uint32_t i = 0; i < 4u; i++) {
        ...
        glUniform4f(gl.loc_color, (i & 1u) ? 0.87f : 0.82f,
                    (i & 1u) ? 0.94f : 0.91f, 0.99f, 0.97f);
        glDrawArrays(GL_TRIANGLES, cup_history_quads[i] * 6, 6);
      }
      use_rounded_rect(&cup_action_frame_style);
      for (uint32_t i = 0; i < 5u; i++) {
        if (!cup_action_frame_quads[i]) continue;
      ...
      for (uint32_t i = 0; i < 5u; i++) {
        if (!cup_action_quads[i]) continue;
        const int selected = !competition_frontend_cup_bracket_editing() &&
                             competition_display_focus == i;
        const int enabled = competition_frontend_item_enabled(i);
        glUniform4f(gl.loc_color,
      ...
      glUniform1f(gl.loc_image, 0.0f);
      glBindTexture(GL_TEXTURE_2D, gl.efootball_tex);
      glUniform4f(gl.loc_color, 0.99f, 0.99f, 1.0f, 1.0f);
      glDrawArrays(GL_TRIANGLES, cup_title_first * 6, cup_title_count * 6);
      glUniform4f(gl.loc_color, 0.03f, 0.09f, 0.17f, 1.0f);
      if (cup_champion_ribbon_text_count) {
        const int before = cup_champion_ribbon_text_first - cup_text_first;
        const int after_first = cup_champion_ribbon_text_first +
                                cup_champion_ribbon_text_count;
        const int after = cup_text_first + cup_text_count - after_first;
        if (before)
          glDrawArrays(GL_TRIANGLES, cup_text_first * 6, before * 6);
        if (after)
          glDrawArrays(GL_TRIANGLES, after_first * 6, after * 6);
        glUniform4f(gl.loc_color, 0.99f, 0.96f, 0.81f, 1.0f);
        glDrawArrays(GL_TRIANGLES,
                     cup_champion_ribbon_text_first * 6,
                     cup_champion_ribbon_text_count * 6);
      } else
        glDrawArrays(GL_TRIANGLES, cup_text_first * 6, cup_text_count * 6);
      for (uint32_t i = 0; i < 5u; i++) {
        if (!cup_action_text_count[i]) continue;
      ...
      if (cup_rule_scrim_quad) {
        glBindTexture(GL_TEXTURE_2D, gl.tex);
        glUniform1f(gl.loc_solid, 1.0f);
        use_rounded_rect(NULL);
        glUniform4f(gl.loc_color, 0.01f, 0.03f, 0.09f, 0.72f);
        glDrawArrays(GL_TRIANGLES, cup_rule_scrim_quad * 6, 6);
        use_rounded_rect(&cup_rule_card_style);
        glUniform4f(gl.loc_color, 0.93f, 0.97f, 1.0f, 0.99f);
        glDrawArrays(GL_TRIANGLES, cup_rule_card_quad * 6, 6);
        use_rounded_rect(NULL);
        glUniform4f(gl.loc_color, 0.025f, 0.10f, 0.28f, 1.0f);
        glDrawArrays(GL_TRIANGLES, cup_rule_header_quad * 6, 6);
        use_rounded_rect(&cup_rule_ok_style);
        glUniform4f(gl.loc_color, 0.99f, 0.88f, 0.07f, 1.0f);
        glDrawArrays(GL_TRIANGLES, cup_rule_ok_quad * 6, 6);
        use_rounded_rect(NULL);
        glUniform1f(gl.loc_solid, 0.0f);
        glBindTexture(GL_TEXTURE_2D, gl.efootball_tex);
        glUniform4f(gl.loc_color, 1.0f, 1.0f, 1.0f, 1.0f);
        glDrawArrays(GL_TRIANGLES, cup_rule_title_first * 6,
                     cup_rule_title_count * 6);
        glUniform4f(gl.loc_color, 0.04f, 0.12f, 0.26f, 1.0f);
        glDrawArrays(GL_TRIANGLES, cup_rule_body_first * 6,
                     cup_rule_body_count * 6);
        glDrawArrays(GL_TRIANGLES, cup_rule_ok_text_first * 6,
                     cup_rule_ok_text_count * 6);
      }
      glBindTexture(GL_TEXTURE_2D, gl.tex);
      glUniform1f(gl.loc_solid, 1.0f);
    } else if (competition_settings_page) {
      ...
      if (competition_cup_logo_quad) {
        ...
        glBindTexture(GL_TEXTURE_2D,
                      logo_tex ? logo_tex : gl.main_menu_brand_tex);
        glDrawArrays(GL_TRIANGLES, competition_cup_logo_quad * 6, 6);
        glBindTexture(GL_TEXTURE_2D, gl.tex);
        glUniform1f(gl.loc_image, 0.0f);
        glUniform1f(gl.loc_solid, 1.0f);
      }
      ...
      const uint32_t action_focus =
          competition_display_state == COMPETITION_FRONTEND_CUP_SETTINGS
              ? 5u
              : competition_display_state ==
                    COMPETITION_FRONTEND_LEAGUE_SETTINGS ? 4u
              : competition_frontend_cup_player_count();
  ...
  if (set_piece_selector || cup_settings_popup || cup_general_popup ||
      custom_2p_team_selector ||
      custom_2p_prematch_hub_raw || custom_2p_transition)
    glBindTexture(GL_TEXTURE_2D, gl.efootball_tex);
  ...
  if (custom_white_text_quads) {
    ...
    if (cup_settings_popup || cup_general_popup || custom_2p_team_selector ||
        custom_2p_prematch_hub_raw ||
        custom_2p_transition)
      glBindTexture(GL_TEXTURE_2D, gl.tex);
  }
