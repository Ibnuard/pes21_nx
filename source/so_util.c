...
static Handle current_process_handle(void) {
  const Handle environment_handle = envGetOwnProcessHandle();
  return environment_handle == INVALID_HANDLE ? CUR_PROCESS_HANDLE
                                               : environment_handle;
}
...
void so_finalize(so_module *mod) {
  Result rc = 0;
  const uintptr_t syms_offset =
      (uintptr_t)mod->syms - (uintptr_t)mod->load_base;
  const uintptr_t dynstr_offset =
      (uintptr_t)mod->dynstrtab - (uintptr_t)mod->load_base;

  // map the entire thing as code memory
  debugPrintf("%s: map code %p <- %p, %u KB\n", mod->name,
              mod->load_virtbase, mod->load_base,
              (u32)(mod->load_size / 1024));
  rc = svcMapProcessCodeMemory(current_process_handle(), (u64)mod->load_virtbase, (u64)mod->load_base, mod->load_size);
  if (R_FAILED(rc)) fatal_error("Error: svcMapProcessCodeMemory failed:\n%08x", rc);
  debugPrintf("%s: code mapping done\n", mod->name);
  ...
  for (int want_x = 1; want_x >= 0; want_x--) {
    ...
    while (pg < num_pages) {
      ...
      rc = svcSetProcessMemoryPermission(current_process_handle(), addr, size, want_x ? Perm_Rx : Perm_Rw);
  ...
  free(is_x_page);
  // The source pages become inaccessible code-memory backing after mapping.
  // Keep later export lookups on the readable runtime mapping instead.
  mod->syms = (Elf64_Sym *)((uintptr_t)mod->load_virtbase + syms_offset);
  mod->dynstrtab = (char *)((uintptr_t)mod->load_virtbase + dynstr_offset);
  debugPrintf("%s: permissions finalized\n", mod->name);
}
  ...
  for (int i = 0; i < mod->elf_hdr->e_shnum; i++) {
    ...
    if (strcmp(sh_name, ".init_array") == 0) {
      ...
      for (int j = 0; j < mod->sec_hdr[i].sh_size / 8; j++) {
        if (init_array[j] != 0) {
          if (j < 16 || (j % 64) == 0)
            debugPrintf("%s: init_array[%d/%u] = %p\n", mod->name, j,
                        (u32)(mod->sec_hdr[i].sh_size / 8), init_array[j]);
          init_array[j]();
        }
      }
    }
...
int so_unload(so_module *mod) {
  ...
  for (int i = 0; i < mod->phnum; i++) {
    ...
    svcSetProcessMemoryPermission(current_process_handle(), seg_start, seg_end - seg_start, Perm_Rw);
  }
  // unmap everything
  svcUnmapProcessCodeMemory(current_process_handle(), (u64)mod->load_virtbase, (u64)mod->load_base, mod->load_size);
