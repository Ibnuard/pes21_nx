...
from generate_legacy_player_cleanup import (
    build_cleanup as build_legacy_cleanup,
    render_report as render_legacy_cleanup_report,
)
from pesdb import (
    PlayerAssignment,
    decode_wesys,
...
def build_catalog(
    ...
    dict[str, Any], dict[int, list[PlayerAssignment]], dict[str, Any]
]:
    ...
    cleaned_rosters = {
        team_id: list(pes21_rosters[team_id]) for team_id in safe_team_ids
    }
    removals_by_team: dict[int, set[int]] = {}
    for row in cleanup["removals"]:
        team_id = int(row["fallback_team_id"])
        player_id = int(row["player_id"])
        removals_by_team.setdefault(team_id, set()).add(player_id)
    for team_id, removed_player_ids in sorted(removals_by_team.items()):
        if team_id not in all_team_ids:
            raise RuntimeError(f"cleanup targets non-catalog team {team_id}")
        base_roster = pes21_rosters.get(team_id)
        if not base_roster:
            raise RuntimeError(f"cleanup team {team_id} has no PES21 roster")
        present_ids = {assignment.player_id for assignment in base_roster}
        missing_ids = removed_player_ids - present_ids
        if missing_ids:
            raise RuntimeError(
                f"cleanup team {team_id} does not contain player IDs: "
                + ", ".join(map(str, sorted(missing_ids)))
            )
        roster = [
            assignment
            for assignment in base_roster
            if assignment.player_id not in removed_player_ids
        ]
        if len(roster) < minimum_players:
            raise RuntimeError(
                f"cleanup leaves team {team_id} with only {len(roster)} players"
            )
        cleaned_rosters[team_id] = roster
    return catalog, cleaned_rosters, cleanup
...
def render_roster_include(
    catalog: dict[str, Any],
    rosters: dict[int, list[PlayerAssignment]],
    ...
    lines = [
        "// Generated by tools/generate_exhibition_team_catalog.py.",
        f"// Catalog content ID: {catalog['content_id']}",
        f"// Legacy cleanup content ID: {cleanup['content_id']}",
        "// Native PES21 fallbacks plus EF10-driven legacy cleanup overrides.",
        "// Do not edit manually.",
        "",
...
def render_report(
    catalog: dict[str, Any], ef10_dir: Path, cleanup: dict[str, Any]
) -> str:
    ...
    lines = [
        ...
        f"table removes {cleanup['counts']['removed_legacy_memberships']} stale club memberships",
        f"from {cleanup['counts']['affected_fallback_clubs']} old clubs by stable player ID; national-team",
...
def main() -> None:
    ...
    parser.add_argument(
        "--legacy-cleanup", type=Path, default=DEFAULT_LEGACY_CLEANUP
    )
    parser.add_argument(
        "--legacy-cleanup-report",
        type=Path,
        default=DEFAULT_LEGACY_CLEANUP_REPORT,
    )
    parser.add_argument(
    ...
    catalog, rosters, cleanup = build_catalog(args)
    root = args.root.resolve()
    outputs = {
        ...
        resolve_from_root(root, args.rosters_output): render_roster_include(
            catalog, rosters, cleanup
        ),
        resolve_from_root(root, args.legacy_cleanup): json.dumps(
            cleanup, indent=2, ensure_ascii=True
        ),
        resolve_from_root(
            root, args.legacy_cleanup_report
        ): render_legacy_cleanup_report(cleanup, catalog),
        resolve_from_root(root, args.report_output): render_report(
            catalog, resolve_from_root(root, args.ef10_dir), cleanup
        ),
    }
